...
npm run auth:video-affiliate:worker
npm run dry-run:video-affiliate:worker
npm run deploy:video-affiliate:worker
npm run status:video-affiliate:worker
npm run logs:video-affiliate:worker
...
This project is indexed by GitNexus as **shopping-affiliate** (16071 symbols, 31604 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
