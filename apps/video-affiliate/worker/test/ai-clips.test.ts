...
test('buildAiClipProcessingQueueJob produces a legacy _queue-compatible job', () => {
    ...
    // AI/manual Media clips use the normal subtitle-burn flow — the durable queue job carries
    // NO skipSubtitles flag, so processNextInQueue defaults it to false and centered subtitles
    // are burned in (matching legacy inbox/cadence jobs).
    assert.equal('skipSubtitles' in job, false)
})
