...
function getSessionTokenFromRequest(c: Context<any>): string {
    const cookies = parseCookieHeader(String(c.req.header('cookie') || ''))
    const cookieToken = String(cookies[SESSION_COOKIE_NAME] || '').trim()
    if (cookieToken.startsWith('sess_')) return cookieToken

    const headerToken = String(c.req.header('x-auth-token') || '').trim()
    if (headerToken.startsWith('sess_')) return headerToken

    return ''
}

function buildSessionCookie(sessionToken: string, maxAgeSeconds = SESSION_COOKIE_MAX_AGE_SECONDS): string {
    const normalizedToken = String(sessionToken || '').trim()
    if (!normalizedToken.startsWith('sess_')) return buildClearedSessionCookie()
    return [
        `${SESSION_COOKIE_NAME}=${encodeURIComponent(normalizedToken)}`,
        'Path=/',
...
function resolveCorsOrigin(requestOrigin: string | undefined, env: Env): string {
    ...
    if (normalizedOrigin) {
        try {
            ...
            if (
                ...
                || hostname.endsWith('.video-affiliate-webapp.pages.dev')
            ) {
...
app.use('*', async (c, next) => {
    const corsMiddleware = cors({
        origin: (requestOrigin) => resolveCorsOrigin(requestOrigin, c.env),
...
app.use('*', async (c, next) => {
    ...
    if (path === '/api/me') {
        ...
        c.header('Cache-Control', 'private, max-age=120')
        c.header('Vary', 'x-auth-token')
    } else if (path === '/api/inbox' || path === '/api/inbox/system') {
        c.header('Cache-Control', 'private, max-age=15')
        c.header('Vary', 'x-auth-token')
    }
})

async function isSystemAdmin(db: D1Database, email: string): Promise<boolean> {
    const row = await db.prepare('SELECT 1 AS ok FROM system_admins WHERE email = ? LIMIT 1')
        .bind(String(email || '').trim().toLowerCase()).first()
    return !!row
}

async function isNamespaceOwner(db: D1Database, email: string): Promise<boolean> {
    const row = await db.prepare('SELECT 1 AS ok FROM email_namespaces WHERE email = ? LIMIT 1')
        .bind(String(email || '').trim().toLowerCase()).first()
    return !!row
}

async function isTeamMemberEmail(db: D1Database, email: string): Promise<boolean> {
    const normalizedEmail = String(email || '').trim().toLowerCase()
    if (!normalizedEmail) return false
    const row = await db.prepare('SELECT 1 AS ok FROM team_members WHERE lower(trim(email)) = ? LIMIT 1')
...
async function isNamespaceOwnerIdentity(db: D1Database, email: string, namespaceId: string): Promise<boolean> {
    const normalizedNamespaceId = String(namespaceId || '').trim()
    if (!normalizedNamespaceId) return false

    const normalizedEmail = String(email || '').trim().toLowerCase()
    if (await isTeamMemberEmail(db, normalizedEmail)) return false

    const hasLineOwner = normalizedEmail ? await db.prepare(
        `SELECT 1 AS ok
         FROM line_users
         WHERE namespace_id = ?
           AND lower(trim(email)) = ?
         LIMIT 1`
    ).bind(normalizedNamespaceId, normalizedEmail).first().catch(() => null) : null
    if (hasLineOwner) return true

    if (!normalizedEmail) return false
    const mapped = await db.prepare(
        'SELECT 1 AS ok FROM email_namespaces WHERE email = ? AND namespace_id = ? LIMIT 1'
    ).bind(normalizedEmail, normalizedNamespaceId).first().catch(() => null)
    return !!mapped
}
...
async function requireOwnerSession(c: Context<{ Bindings: Env, Variables: { botId: string; bucket: R2Bucket } }>) {
    ...
    const user = await c.env.DB.prepare('SELECT email, namespace_id FROM users WHERE session_token = ?').bind(token).first() as { email?: string; namespace_id?: string } | null
    if (!user?.namespace_id) {
        return { ok: false as const, response: c.json({ error: 'Unauthorized' }, 401) }
    }
    const ownerOk = await isNamespaceOwnerIdentity(c.env.DB, String(user.email || ''), String(user.namespace_id || ''))
    if (!ownerOk) {
...
async function requireSystemAdminSession(c: Context<{ Bindings: Env, Variables: { botId: string; bucket: R2Bucket } }>) {
    ...
    const user = await c.env.DB.prepare(
        'SELECT email, namespace_id FROM users WHERE session_token = ?'
    ).bind(token).first().catch(() => null) as { email?: string; namespace_id?: string } | null
    if (!user?.email || !user?.namespace_id) {
        return { ok: false as const, response: c.json({ error: 'Unauthorized' }, 401) }
    }
    const sysAdmin = await isSystemAdmin(c.env.DB, String(user.email))
    if (!sysAdmin) {
        return { ok: false as const, response: c.json({ error: 'Forbidden' }, 403) }
    }
    return {
        ...
        namespaceId: String(user.namespace_id || '').trim(),
    }
}

async function upsertUserWorkspaceByEmail(db: D1Database, emailRaw: string, namespaceId: string): Promise<void> {
    ...
    ).bind(email).first().catch(() => null) as { ok?: number } | null

    if (existing?.ok) {
        await db.prepare(
            'UPDATE users SET namespace_id = ? WHERE lower(trim(email)) = ?'
        ).bind(ns, email).run().catch(() => { })
        return
    }

    await db.prepare(
        "INSERT INTO users (email, namespace_id, session_token) VALUES (?, ?, '')"
    ).bind(email, ns).run().catch(() => { })
}

async function resolveManagedWorkspaceForRole(
    db: D1Database,
    emailRaw: string,
    ...
    const email = String(emailRaw || '').trim().toLowerCase()
    const currentNamespace = String(currentNamespaceId || '').trim()
    const adminNamespace = String(adminNamespaceId || '').trim()
    ...
    if (mappedNamespace) return mappedNamespace

    if (currentNamespace && currentNamespace !== adminNamespace) return currentNamespace
    return createOwnerWorkspaceId()
}

async function assignLineUserRole(
    db: D1Database,
    lineUserIdRaw: string,
    targetRoleRaw: string,
    adminNamespaceIdRaw: string
): Promise<{
    line_user_id: string
    namespace_id: string
    role: 'admin' | 'member' | 'team'
    status: 'approved'
  }> {
    const lineUserId = String(lineUserIdRaw || '').trim()
    const adminNamespaceId = String(adminNamespaceIdRaw || '').trim()
    const targetRole = String(targetRoleRaw || '').trim().toLowerCase()
    if (!lineUserId) throw new Error('line_user_id required')
    if (!adminNamespaceId) throw new Error('admin_namespace_id required')
    if (!['admin', 'member', 'team'].includes(targetRole)) throw new Error('invalid_role')

    await ensureLineUsersTable(db)
    const lineUser = await db.prepare(
        'SELECT email, namespace_id FROM line_users WHERE line_user_id = ? LIMIT 1'
    ).bind(lineUserId).first().catch(() => null) as { email?: string; namespace_id?: string } | null

    const email = String(lineUser?.email || '').trim().toLowerCase()
    const currentNamespaceId = String(lineUser?.namespace_id || '').trim()
    if (!email) throw new Error('member_not_found')
    ...
    if (targetRole === 'team') {
        ...
        await db.prepare('DELETE FROM team_members WHERE lower(trim(email)) = ?').bind(email).run().catch(() => { })
        if (targetRole === 'admin') {
            await db.prepare('INSERT OR IGNORE INTO system_admins (email) VALUES (?)').bind(email).run().catch(() => { })
        } else {
            await db.prepare('DELETE FROM system_admins WHERE lower(trim(email)) = ?').bind(email).run().catch(() => { })
        }
    }
    ...
    await db.prepare(
        `UPDATE line_users
         SET namespace_id = ?, status = 'approved', updated_at = datetime('now')
         WHERE line_user_id = ?`
    ).bind(targetNamespaceId, lineUserId).run().catch(() => { })
    await upsertUserWorkspaceByEmail(db, email, targetNamespaceId)

    return {
        line_user_id: lineUserId,
        namespace_id: targetNamespaceId,
        role: targetRole as 'admin' | 'member' | 'team',
        status: 'approved',
    }
}
...
async function upsertUserSessionByTelegram(
    db: D1Database,
    telegramId: string,
    email: string,
    namespaceId: string,
    ...
    const telegramIdValue = Number(String(telegramId || '').trim())
    ...
    const existingTgRow = await db.prepare(
        'SELECT email FROM users WHERE telegram_id = ?'
    ).bind(telegramIdValue).first() as { email?: string } | null

    if (existingTgRow) {
        await db.prepare(
            'UPDATE users SET email = ?, namespace_id = ?, session_token = ? WHERE telegram_id = ?'
        ).bind(email, namespaceId, sessionToken, telegramIdValue).run()
        return
    }

    await db.prepare(
        'INSERT INTO users (telegram_id, email, namespace_id, session_token) VALUES (?, ?, ?, ?)'
    ).bind(telegramIdValue, email, namespaceId, sessionToken).run()
}

async function bindTelegramToExistingSession(
    db: D1Database,
    sessionToken: string,
    telegramId: string,
    botScope: string,
    ...
    if (!normalizedSessionToken.startsWith('sess_') || !normalizedTelegramId || !normalizedBotScope) return null

    const currentUser = await db.prepare(
        'SELECT email, namespace_id, session_token FROM users WHERE session_token = ? LIMIT 1'
    ).bind(normalizedSessionToken).first() as { email?: string; namespace_id?: string; session_token?: string } | null

    const email = String(currentUser?.email || '').trim().toLowerCase()
    const namespaceId = String(currentUser?.namespace_id || '').trim()
    const effectiveSessionToken = String(currentUser?.session_token || '').trim() || normalizedSessionToken
    if (!email || !namespaceId || namespaceId !== normalizedBotScope) return null

    await upsertUserSessionByTelegram(db, normalizedTelegramId, email, namespaceId, effectiveSessionToken)
    await upsertTelegramBotSession(db, normalizedTelegramId, normalizedBotScope, email, namespaceId, effectiveSessionToken)
    return {
...
async function hasTelegramWorkspaceAccess(
    db: D1Database,
    telegramId: string,
    botScope: string,
): Promise<boolean> {
    const normalizedTelegramId = String(telegramId || '').trim()
    const normalizedBotScope = String(botScope || '').trim()
    if (!normalizedTelegramId || !normalizedBotScope) return false

    const scoped = await getTelegramBotSession(db, normalizedTelegramId, normalizedBotScope).catch(() => null)
    if (String(scoped?.email || '').trim()) return true

    const telegramIdValue = Number(normalizedTelegramId)
    if (!Number.isFinite(telegramIdValue)) return false

    const user = await db.prepare(
        'SELECT email FROM users WHERE telegram_id = ? AND namespace_id = ? LIMIT 1'
    ).bind(telegramIdValue, normalizedBotScope).first() as { email?: string } | null
...
async function resolveOwnerEmailForNamespace(db: D1Database, namespaceId: string): Promise<string> {
    const normalizedNamespaceId = String(namespaceId || '').trim()
    ...
    try {
        const mapped = await db.prepare(
            `SELECT email
             FROM email_namespaces
             WHERE namespace_id = ?
             ORDER BY datetime(updated_at) DESC, datetime(created_at) DESC
             LIMIT 1`
        ).bind(normalizedNamespaceId).first() as { email?: string } | null
        const mappedEmail = String(mapped?.email || '').trim().toLowerCase()
        if (mappedEmail) return mappedEmail
    } catch {
        // ignore older db states
    }
    ...
    const user = await db.prepare(
        `SELECT email
         ...
         ORDER BY datetime(created_at) ASC, telegram_id ASC
         LIMIT 1`
    ).bind(normalizedNamespaceId).first() as { email?: string } | null
    return String(user?.email || '').trim().toLowerCase()
}

async function claimOrValidateChannelOwner(
    db: D1Database,
    botId: string,
    telegramId: string,
    displayName?: string,
): Promise<{ allowed: boolean; claimed: boolean; ownerTelegramId: string }> {
    const normalizedBotId = String(botId || '').trim()
    const normalizedTelegramId = String(telegramId || '').trim()
    if (!normalizedBotId || !normalizedTelegramId) {
        return { allowed: false, claimed: false, ownerTelegramId: '' }
    }

    const channel = await db.prepare(
        'SELECT owner_telegram_id FROM channels WHERE bot_id = ? LIMIT 1'
    ).bind(normalizedBotId).first() as { owner_telegram_id?: string | number } | null

    if (!channel) {
        return { allowed: false, claimed: false, ownerTelegramId: '' }
    }

    const currentOwnerTelegramId = String(channel.owner_telegram_id || '').trim()
    if (currentOwnerTelegramId && currentOwnerTelegramId !== normalizedTelegramId) {
        return { allowed: false, claimed: false, ownerTelegramId: currentOwnerTelegramId }
    }

    let claimed = false
    if (!currentOwnerTelegramId) {
        await db.prepare(
            "UPDATE channels SET owner_telegram_id = ? WHERE bot_id = ? AND TRIM(COALESCE(owner_telegram_id, '')) = ''"
        ).bind(normalizedTelegramId, normalizedBotId).run()
        claimed = true
    }

    const resolvedOwnerTelegramId = claimed ? normalizedTelegramId : currentOwnerTelegramId
    const telegramIdValue = Number(normalizedTelegramId)
    if (Number.isFinite(telegramIdValue)) {
        await db.prepare(
            'INSERT INTO allowed_users (telegram_id, name) VALUES (?, ?) ON CONFLICT DO NOTHING'
        ).bind(telegramIdValue, String(displayName || 'owner').trim() || 'owner').run().catch(() => { })
    }

    return {
        allowed: true,
        claimed,
        ownerTelegramId: resolvedOwnerTelegramId || normalizedTelegramId,
    }
}

async function provisionTelegramOwnerSession(
    db: D1Database,
    telegramId: string,
    botScope: string,
    displayName?: string,
    ...
    const normalizedTelegramId = String(telegramId || '').trim()
    const normalizedBotScope = String(botScope || '').trim()
    if (!normalizedTelegramId || !normalizedBotScope) return null

    const ownerAccess = await claimOrValidateChannelOwner(db, normalizedBotScope, normalizedTelegramId, displayName)
    if (!ownerAccess.allowed) return null

    const ownerEmail = await resolveOwnerEmailForNamespace(db, normalizedBotScope)
    if (!ownerEmail) return null

    await upsertOwnerNamespaceMapping(db, ownerEmail, normalizedBotScope).catch(() => { })

    const sessionToken = 'sess_' + crypto.randomUUID().replace(/-/g, '')
    const telegramIdValue = Number(normalizedTelegramId)
    const existingTgRow = Number.isFinite(telegramIdValue)
        ? await db.prepare('SELECT email FROM users WHERE telegram_id = ?').bind(telegramIdValue).first() as { email?: string } | null
        : null

    if (existingTgRow) {
        await db.prepare(
            'UPDATE users SET email = ?, namespace_id = ?, session_token = ? WHERE telegram_id = ?'
        ).bind(ownerEmail, normalizedBotScope, sessionToken, telegramIdValue).run()
    } else if (Number.isFinite(telegramIdValue)) {
        await db.prepare(
            'INSERT INTO users (telegram_id, email, namespace_id, session_token) VALUES (?, ?, ?, ?)'
        ).bind(telegramIdValue, ownerEmail, normalizedBotScope, sessionToken).run()
    } else {
        await db.prepare(
            'INSERT INTO users (email, namespace_id, session_token) VALUES (?, ?, ?)'
        ).bind(ownerEmail, normalizedBotScope, sessionToken).run()
    }

    await upsertTelegramBotSession(db, normalizedTelegramId, normalizedBotScope, ownerEmail, normalizedBotScope, sessionToken)

    return {
        session_token: sessionToken,
        namespace_id: normalizedBotScope,
        email: ownerEmail,
        claimed_owner: ownerAccess.claimed,
    }
}
...
app.post('/api/pages/profile-token-health', async (c) => {
    ...
    if (inputProfiles.length === 0) {
        return c.json({ success: true, namespace_id: namespaceId, profiles: [] })
    }

    const pages = (((await c.env.DB.prepare(
        'SELECT id, name, access_token FROM pages WHERE bot_id = ?'
    ).bind(namespaceId).all()).results || []) as Array<{ id?: string; name?: string; access_token?: string | null }>)

    const pagesById = new Map<string, { id: string; name: string; access_token: string }>()
    const pagesByName = new Map<string, Array<{ id: string; name: string; access_token: string }>>()
    for (const row of pages) {
        ...
        const pageRecord = { id, name, access_token: accessToken }
        pagesById.set(id, pageRecord)
        if (!pagesByName.has(normalizedName)) pagesByName.set(normalizedName, [])
        pagesByName.get(normalizedName)!.push(pageRecord)
    }

    const linkedProfiles = await getNamespaceLinkedTaggedProfiles(c.env.DB, namespaceId)
    const tokenPool = await getNamespacePagesTokenPool(c.env.DB, namespaceId)

    // Query latest post per page to check for actual post failures
    const latestPostByPage = new Map<string, { status: string; error_message: string }>()
    try {
        const { results: latestPosts } = await c.env.DB.prepare(
            `SELECT page_id, status, error_message FROM post_history WHERE bot_id = ? AND id IN (SELECT MAX(id) FROM post_history WHERE bot_id = ? GROUP BY page_id)`
        ).bind(namespaceId, namespaceId).all<{ page_id: string; status: string; error_message: string | null }>()
        for (const row of (latestPosts || [])) {
            const pid = String(row?.page_id || '').trim()
            if (pid) latestPostByPage.set(pid, { status: String(row?.status || '').trim(), error_message: String(row?.error_message || '').trim() })
        }
    } catch (err) {
        console.log(`[PROFILE-TOKEN-HEALTH] failed to query latest post_history: ${String(err)}`)
    }

    const results: BrowserSavingProfileTokenHealthResult[] = []

    for (const rawProfile of inputProfiles as BrowserSavingProfileRecord[]) {
        const profileId = String(rawProfile?.id || '').trim()
        if (!profileId) continue

        const profileName = String(rawProfile?.name || '').trim() || profileId
        const tags = normalizeBrowserSavingProfileTags(rawProfile?.tags)
        const wantsPost = tags.includes('post')
        const wantsComment = tags.includes('comment')
        const localPostcronToken = pickProfilePostcronToken(rawProfile)
        const localCommentToken = pickProfileCommentToken(rawProfile)
        const localPageId = parsePageIdFromAvatarUrl(String(rawProfile?.page_avatar_url || ''))
        const localPageName = getExplicitTaggedProfilePageName(rawProfile)
        const normalizedLocalPageName = normalizePageName(localPageName)

        let matchedPage = localPageId ? pagesById.get(localPageId) : undefined
        if (!matchedPage && normalizedLocalPageName) {
            const namedMatches = pagesByName.get(normalizedLocalPageName) || []
            if (namedMatches.length > 0) matchedPage = namedMatches[0]
        }

        const pageId = String(matchedPage?.id || localPageId || '').trim()
        const pageName = String(matchedPage?.name || localPageName || '').trim()
        const pageFound = !!matchedPage

        let hasPagePostToken = false
        let hasPageCommentToken = false
        let explicitlyLinked = false

        if (matchedPage) {
            const linkedEntryExists = hasLinkedTaggedProfilesEntryForPage(linkedProfiles, matchedPage.id)
            const linkedProfileIds = linkedEntryExists
                ? getLinkedTaggedProfileIdsForPage(linkedProfiles, matchedPage.id)
                : new Set<string>()
            explicitlyLinked = linkedEntryExists ? linkedProfileIds.has(profileId.toLowerCase()) : false

            const pagePoolEntry = tokenPool[matchedPage.id] || { post_tokens: [], comment_tokens: [] }
            hasPagePostToken = normalizePostTokenPool([
                String(matchedPage.access_token || '').trim(),
                ...((pagePoolEntry.post_tokens || []).map((item) => String(item || '').trim())),
            ]).length > 0
            hasPageCommentToken = normalizeCommentTokenPool(
                (pagePoolEntry.comment_tokens || []).map((item) => String(item || '').trim()),
            ).length > 0
        }

        // Only flag as issue when the latest post for this page actually failed
        const latestPost = pageId ? latestPostByPage.get(pageId) : undefined
        const hasPostFailure = latestPost?.status === 'failed'
        const issues: string[] = []
        if (hasPostFailure) {
            issues.push(latestPost.error_message || 'โพสต์ล่าสุดไม่สำเร็จ')
        }
        ...
        results.push({
            ...
            has_issue: hasPostFailure,
            issues,
            issue_summary: issues[0] || '',
            has_local_postcron_token: !!localPostcronToken,
            has_local_comment_token: !!localCommentToken,
            has_page_post_token: hasPagePostToken,
            has_page_comment_token: hasPageCommentToken,
            explicitly_linked: explicitlyLinked,
        })
    }
...
app.put('/api/r2-upload/:key{.+}', async (c) => {
    // Auth: accept session cookie/header token (sess_*) or bot token
    const rawHeaderToken = String(c.req.header('x-auth-token') || '').trim()
    const authToken = getSessionTokenFromRequest(c)
    let authorized = false
    if (rawHeaderToken === c.env.TELEGRAM_BOT_TOKEN) {
        authorized = true
    ...
    const fallbackBody = requestBody ? null : await c.req.arrayBuffer()
    const contentLength = Number(c.req.header('content-length') || 0)

    await c.get('bucket').put(key, requestBody || fallbackBody || new ArrayBuffer(0), {
...
app.get('/api/inbox-cover/:videoId/preview', async (c) => {
    const videoId = String(c.req.param('videoId') || '').trim()
    const namespaceId = String(c.req.query('namespace_id') || '').trim()
    if (!videoId) return c.json({ error: 'video_id_required' }, 400)
    ...
    const jpgKey = `_inbox_cover/${videoId}_preview.jpg`
    const webpKey = `_inbox_cover/${videoId}.webp`

    const obj = await bucket.get(jpgKey).catch(() => null) || await bucket.get(webpKey).catch(() => null)
    if (!obj) return c.json({ error: 'not found' }, 404)

    const headers = new Headers()
    headers.set('Content-Type', obj.httpMetadata?.contentType || (String(obj.key || '').endsWith('.jpg') ? 'image/jpeg' : 'image/webp'))
...
function pickOldestGalleryVideoForPosting(videos: Array<Record<string, unknown>>): Record<string, unknown> | null {
    let picked: Record<string, unknown> | null = null
    let pickedTs = Number.POSITIVE_INFINITY

    for (const video of videos || []) {
        const ts = getGalleryVideoSortMs(video)
        if (!picked) {
            picked = video
            pickedTs = ts
            continue
        ...
        if (ts === pickedTs) {
            ...
            if (nextId && (!pickedId || nextId.localeCompare(pickedId) < 0)) {
                picked = video
                pickedTs = ts
            }
        }
...
function pickNewestGalleryVideoForPosting(videos: Array<Record<string, unknown>>): Record<string, unknown> | null {
    let picked: Record<string, unknown> | null = null
...
function pickRandomGalleryVideoForPostingOrder(videos: Array<Record<string, unknown>>): Record<string, unknown> | null {
...
function pickGalleryVideoForPostingByOrder(
    ...
    switch (postingOrder) {
        case 'newest_first':
            return pickNewestGalleryVideoForPosting(videos)
        case 'random':
            return pickRandomGalleryVideoForPostingOrder(videos)
        case 'oldest_first':
        default:
            return pickOldestGalleryVideoForPosting(videos)
    }
}
...
function normalizeVideoDuplicateValue(value: unknown): string {
    return String(value || '').trim().replace(/#.*$/, '')
}

function getNamespaceVideoDuplicateKey(video: Record<string, unknown>): string {
    const namespaceId = String(video.namespace_id || video.namespaceId || '').trim()
    const originalUrl = normalizeVideoDuplicateValue(video.originalUrl || video.original_url)
    if (namespaceId && originalUrl) return `${namespaceId}::original::${originalUrl}`

    const sourceType = String(video.sourceType || video.source_type || '').trim().toLowerCase()
    const sourceLabel = normalizeVideoDuplicateValue(video.sourceLabel || video.source_label)
    const videoUrl = normalizeVideoDuplicateValue(video.videoUrl || video.video_url)
    const previewUrl = normalizeVideoDuplicateValue(video.previewUrl || video.preview_url)

    if (namespaceId && sourceType === 'xhs_url' && sourceLabel) return `${namespaceId}::xhs::${sourceLabel}`
    if (namespaceId && videoUrl) return `${namespaceId}::video::${videoUrl}`
    if (namespaceId && previewUrl) return `${namespaceId}::preview::${previewUrl}`
    return ''
}
...
function dedupeSystemGalleryVideos(videos: any[]): any[] {
    ...
    for (const video of byVideoId.values()) {
        const sourceKey = getNamespaceVideoDuplicateKey(video)
        if (!sourceKey) {
            bySourceKey.set(`id::${String(video.id || '').trim()}`, video)
            continue
        }
        const previous = bySourceKey.get(sourceKey)
        if (!previous) {
            bySourceKey.set(sourceKey, video)
            continue
        }
        bySourceKey.set(sourceKey, pickPreferredSystemGalleryVideo(previous, video))
    }

    return Array.from(bySourceKey.values()).sort((a, b) => getGalleryVideoSortMs(b) - getGalleryVideoSortMs(a))
}

function getNamespaceInventoryGroupKey(video: Record<string, unknown>): string {
    const namespaceId = String(video.namespace_id || video.namespaceId || '').trim()
    const videoId = String(video.id || '').trim()
    const fingerprint = normalizeNamespaceVideoSourceFingerprint(
        video.sourceFingerprint || video.source_fingerprint
    )
    if (namespaceId && fingerprint) return `${namespaceId}::fingerprint::${fingerprint}`
    if (namespaceId && videoId) return `${namespaceId}::id::${videoId}`
    return videoId
}

function getPostedAtMs(video: Record<string, unknown>): number {
    const ts = new Date(String(video.postedAt || video.posted_at || '')).getTime()
    return Number.isFinite(ts) ? ts : 0
}

function pickPreferredNamespaceInventoryVideo(current: Record<string, unknown>, next: Record<string, unknown>): Record<string, unknown> {
    const currentPosted = isNamespaceGalleryVideoPosted(current)
    const nextPosted = isNamespaceGalleryVideoPosted(next)
    if (currentPosted !== nextPosted) {
        return currentPosted ? current : next
    }

    if (currentPosted && nextPosted) {
        const currentPostedAtMs = getPostedAtMs(current)
        const nextPostedAtMs = getPostedAtMs(next)
        if (currentPostedAtMs !== nextPostedAtMs) {
            return nextPostedAtMs >= currentPostedAtMs ? next : current
        }
    }
    ...
    return nextId.localeCompare(currentId) >= 0 ? next : current
}

function dedupeNamespaceInventoryVideos(videos: Array<Record<string, unknown>>): Array<Record<string, unknown>> {
    const grouped = new Map<string, Record<string, unknown>>()
    for (const video of videos || []) {
        const key = getNamespaceInventoryGroupKey(video)
        if (!key) continue
        ...
        grouped.set(key, pickPreferredNamespaceInventoryVideo(previous, video))
    }
    return Array.from(grouped.values()).sort((a, b) => getGalleryVideoSortMs(b) - getGalleryVideoSortMs(a))
}
...
async function pickRandomGalleryVideoForPosting(params: {
    ...
    const candidates = galleryVideos.filter((video) => {
        const id = String(video?.id || '').trim()
        return !!id && allowedIds.has(id) && !params.excludedVideoIds.has(id)
    })
    if (candidates.length > 0) {
        ...
        return {
            ...
            meta: picked,
            shopeeLink: normalizeMetaShopeeLink(picked) || '',
            sourceNamespaceId: String((picked as any).sourceNamespaceId || (picked as any).source_namespace_id || namespaceId).trim() || namespaceId,
        }
    }
...
async function resolveGalleryVideoForRepost(params: {
    env: Env
    fallbackNamespaceId: string
    videoId: string
}): Promise<{ id: string; meta: Record<string, unknown>; shopeeLink: string; sourceNamespaceId: string } | null> {
    const videoId = String(params.videoId || '').trim()
    const fallbackNamespaceId = String(params.fallbackNamespaceId || '').trim() || 'default'
    if (!videoId) return null

    const shortlinkEnabled = await isSystemGalleryEnabledForNamespace(params.env.DB, fallbackNamespaceId)
    if (!shortlinkEnabled) {
        return null
    }

    const namespaceGalleryVideos = await listNamespaceGalleryVideos(params.env, fallbackNamespaceId)
    const namespaceHit = namespaceGalleryVideos.find((video) => String(video.id || '').trim() === videoId)
    if (namespaceHit) {
        return {
            id: videoId,
            meta: namespaceHit,
            shopeeLink: normalizeMetaShopeeLink(namespaceHit as Record<string, unknown>) || '',
            sourceNamespaceId: fallbackNamespaceId,
        }
    }

    await ensureGalleryIndexTable(params.env.DB)
    const galleryRow = await params.env.DB.prepare(
        `SELECT
            namespace_id,
            owner_email,
            script,
         ...
         FROM gallery_index
         WHERE video_id = ?
         ORDER BY CASE WHEN namespace_id = ? THEN 0 ELSE 1 END,
                  COALESCE(updated_at, created_at) DESC,
                  namespace_id ASC
         LIMIT 1`
    ).bind(videoId, fallbackNamespaceId).first() as {
        namespace_id?: string
        owner_email?: string
        script?: string
        title?: string
    ...
    const namespaceCandidates = uniqueVideoIds([
        String(galleryRow?.namespace_id || '').trim(),
        fallbackNamespaceId,
    ])

    for (const namespaceId of namespaceCandidates) {
        if (!namespaceId) continue
        const bucket = namespaceId === fallbackNamespaceId
            ? new BotBucket(params.env.BUCKET, fallbackNamespaceId) as unknown as R2Bucket
            : new BotBucket(params.env.BUCKET, namespaceId) as unknown as R2Bucket

        const metaObj = await bucket.get(`videos/${videoId}.json`)
        if (!metaObj && namespaceId !== String(galleryRow?.namespace_id || '').trim()) {
            continue
        }
        ...
        let meta: Record<string, unknown> = {
            id: videoId,
            namespace_id: namespaceId,
            owner_email: String(galleryRow?.owner_email || '').trim(),
            script: String(galleryRow?.script || '').trim(),
            title: String(galleryRow?.title || '').trim(),
            category: String(galleryRow?.category || '').trim(),
            duration: Number(galleryRow?.duration || 0),
            shopeeLink: String(galleryRow?.shopee_link || '').trim(),
            publicUrl: String(galleryRow?.public_url || '').trim(),
            originalUrl: String(galleryRow?.original_url || '').trim(),
            thumbnailUrl: String(galleryRow?.thumbnail_url || '').trim(),
            createdAt: String(galleryRow?.created_at || '').trim(),
            updatedAt: String(galleryRow?.updated_at || '').trim(),
        }
        if (metaObj) {
            try {
                meta = {
                    ...meta,
                    ...(await metaObj.json() as Record<string, unknown>),
                }
            } catch {
                // Keep index-derived metadata as fallback.
            }
        }

        const shopeeLink = normalizeMetaShopeeLink(meta) || String(galleryRow?.shopee_link || '').trim()
        return {
            id: videoId,
            meta,
            shopeeLink,
...
function isNamespaceAssetUrlForNamespace(url: unknown, namespaceId: string): boolean {
    const normalizedUrl = String(url || '').trim()
    const normalizedNamespaceId = String(namespaceId || '').trim()
    if (!normalizedUrl || !normalizedNamespaceId) return false
    return normalizedUrl.includes(`/${normalizedNamespaceId}/videos/`)
}

async function clearNamespaceProcessedArtifacts(bucket: R2Bucket, videoId: string): Promise<void> {
    const normalizedVideoId = String(videoId || '').trim()
    if (!normalizedVideoId) return
    await Promise.all([
        bucket.delete(`videos/${normalizedVideoId}.json`).catch(() => { }),
        bucket.delete(`videos/${normalizedVideoId}.mp4`).catch(() => { }),
        bucket.delete(getProcessedThumbnailAssetKey(normalizedVideoId)).catch(() => { }),
        bucket.delete(`_processing/${normalizedVideoId}.json`).catch(() => { }),
        bucket.delete(`_queue/${normalizedVideoId}.json`).catch(() => { }),
    ])
}

function isImportedVideoMetaPointingAtSourceNamespace(params: {
    meta: Record<string, unknown>
    targetNamespaceId: string
    sourceNamespaceId: string
}): boolean {
    const targetNamespaceId = String(params.targetNamespaceId || '').trim()
    const sourceNamespaceId = String(params.sourceNamespaceId || '').trim()
    if (!targetNamespaceId || !sourceNamespaceId || targetNamespaceId === sourceNamespaceId) return false

    const meta = params.meta || {}
    const importedFrom = String(meta.importedFrom || meta.sourceNamespaceId || meta.source_namespace_id || '').trim()
    const originalUrl = String(meta.originalUrl || meta.original_url || '').trim()
    const publicUrl = String(meta.publicUrl || meta.public_url || '').trim()
    const thumbnailUrl = String(meta.thumbnailUrl || meta.thumbnail_url || '').trim()

    const pointsToSource =
        isNamespaceAssetUrlForNamespace(originalUrl, sourceNamespaceId)
        || isNamespaceAssetUrlForNamespace(publicUrl, sourceNamespaceId)
        || isNamespaceAssetUrlForNamespace(thumbnailUrl, sourceNamespaceId)
    const pointsToTarget =
        isNamespaceAssetUrlForNamespace(originalUrl, targetNamespaceId)
        || isNamespaceAssetUrlForNamespace(publicUrl, targetNamespaceId)
        || isNamespaceAssetUrlForNamespace(thumbnailUrl, targetNamespaceId)

    return !!(
        (importedFrom && importedFrom === sourceNamespaceId && !pointsToTarget)
        || pointsToSource
    )
}
...
function resolvePostingVideoDownloadUrl(
    ...
    const normalizedNamespaceId = String(namespaceId || '').trim()
    const normalizedVideoId = String(videoId || '').trim()
    const workerAssetUrl = buildWorkerGalleryAssetUrl(
        String(env.WORKER_URL || '').trim(),
        normalizedNamespaceId,
        normalizedVideoId,
        'public',
    )
    if (workerAssetUrl) return workerAssetUrl

    const publicUrl = String(rawPublicUrl || '').trim()
    if (publicUrl && publicUrl.includes(`/${normalizedNamespaceId}/videos/`)) {
        return publicUrl
    }

    return getVideoPublicUrlForNamespace(env.R2_PUBLIC_URL, normalizedNamespaceId, normalizedVideoId)
}
...
async function getAllSystemGalleryVideos(env: Env): Promise<Array<Record<string, unknown>>> {
    const indexedVideos = await listGalleryIndexVideos(env.DB, { requirePublicVideo: true }).catch(() => [])
    ...
    if (cached) {
        ...
        if (createdAt > 0 && (Date.now() - createdAt) < 60_000) {
            return Array.isArray(payload.videos)
                ? payload.videos.filter((video) =>
                    !!String((video as Record<string, unknown>).publicUrl || (video as Record<string, unknown>).public_url || '').trim()
                    && !getBooleanFlag((video as Record<string, unknown>).original_only)
                )
                : []
        }
    }
    ...
    const deduped = dedupeVideosById(videos).filter((video) =>
        !!String((video as Record<string, unknown>).publicUrl || (video as Record<string, unknown>).public_url || '').trim()
        && !getBooleanFlag((video as Record<string, unknown>).original_only)
    ).sort((a, b) => {
...
let namespaceVideoStateColumnsReady: Promise<void> | null = null

async function ensureNamespaceVideoStateColumns(db: D1Database): Promise<void> {
    if (!namespaceVideoStateColumnsReady) {
        namespaceVideoStateColumnsReady = (async () => {
    ...
    await namespaceVideoStateColumnsReady
}
...
async function upsertNamespaceVideoState(db: D1Database, namespaceId: string, videoId: string, fields: Partial<NamespaceVideoStateRow>): Promise<void> {
    await ensureNamespaceVideoStateColumns(db)
    ...
    const next = {
        shopee_link: String(fields.shopee_link ?? current?.shopee_link ?? '').trim(),
        lazada_link: String(fields.lazada_link ?? current?.lazada_link ?? '').trim(),
        shopee_original_link: String(fields.shopee_original_link ?? current?.shopee_original_link ?? '').trim(),
        lazada_original_link: String(fields.lazada_original_link ?? current?.lazada_original_link ?? '').trim(),
        shopee_converted_at: String(fields.shopee_converted_at ?? current?.shopee_converted_at ?? '').trim(),
        lazada_converted_at: String(fields.lazada_converted_at ?? current?.lazada_converted_at ?? '').trim(),
    ...
    await db.prepare(
        ...
        next.shopee_link,
        next.lazada_link,
        next.shopee_original_link,
        next.lazada_original_link,
        next.shopee_converted_at,
        next.lazada_converted_at,
...
async function markNamespaceVideoPosted(db: D1Database, namespaceId: string, videoId: string, postedAt: string): Promise<void> {
...
function normalizeImportedSourceLabel(value: unknown): string {
    return String(value || '').trim().replace(/\s+[•|]\s+imported from\s+.+$/i, '').trim()
}
...
function buildStructuredSourceFingerprint(source: Record<string, unknown> | null | undefined): string {
    const sourceType = String(source?.sourceType || source?.source_type || '').trim().toLowerCase()
    const sourceLabel = normalizeFingerprintUrl(normalizeImportedSourceLabel(source?.sourceLabel || source?.source_label))
    if (sourceType === 'xhs_url' && sourceLabel) {
        return normalizeNamespaceVideoSourceFingerprint(`xhs::${sourceLabel}`)
    }

    const rawVideoUrl = normalizeFingerprintUrl(source?.videoUrl || source?.video_url)
    const looksLikeInternalR2 = /\/videos\/[a-z0-9]+(?:_line)?_original\.mp4(?:\?|$)/i.test(rawVideoUrl)
    if (rawVideoUrl && !looksLikeInternalR2) {
        return normalizeNamespaceVideoSourceFingerprint(`video::${rawVideoUrl}`)
    }

    return ''
}

async function resolveNamespaceVideoSourceFingerprint(params: {
    env: Env
    namespaceId: string
    videoId: string
    bucket?: R2Bucket
    currentState?: NamespaceVideoStateRow | null
    sourceMeta?: Record<string, unknown> | null
}): Promise<string> {
    const namespaceId = String(params.namespaceId || '').trim()
    const videoId = String(params.videoId || '').trim()
    if (!namespaceId || !videoId) return ''

    const existingFingerprint = normalizeNamespaceVideoSourceFingerprint(params.currentState?.source_fingerprint)
    if (existingFingerprint) return existingFingerprint

    const bucket = params.bucket || (new BotBucket(params.env.BUCKET, namespaceId) as unknown as R2Bucket)
    for (const variant of ['original', 'public'] as const) {
        const key = await resolveGalleryAssetKey(bucket, videoId, variant).catch(() => '')
        if (!key) continue
        const head = await bucket.head(key).catch(() => null)
        const etag = normalizeNamespaceVideoSourceFingerprint(head?.httpEtag || '')
        if (etag) {
            return `etag::${etag}::${Number(head?.size || 0)}`
        }
    }

    const metaFingerprint = buildStructuredSourceFingerprint(params.sourceMeta || null)
    if (metaFingerprint) return metaFingerprint

    const inboxObj = await bucket.get(`_inbox/${videoId}.json`).catch(() => null)
    if (inboxObj) {
        const inbox = await inboxObj.json().catch(() => null) as Record<string, unknown> | null
        const inboxFingerprint = buildStructuredSourceFingerprint(inbox)
        if (inboxFingerprint) return inboxFingerprint
    }

    const linkContextObj = await bucket.get(`_link_context/${videoId}.json`).catch(() => null)
    if (linkContextObj) {
        const linkContext = await linkContextObj.json().catch(() => null) as Record<string, unknown> | null
        const linkContextFingerprint = buildStructuredSourceFingerprint(linkContext)
        if (linkContextFingerprint) return linkContextFingerprint
    }

    return ''
}

async function hydrateNamespaceVideoSourceFingerprints(params: {
    env: Env
    namespaceId: string
    ...
    const bucket = params.bucket || (new BotBucket(params.env.BUCKET, namespaceId) as unknown as R2Bucket)
    for (const source of params.sourceVideos) {
        const videoId = String(source.id || '').trim()
        if (!videoId) continue
        const currentRow = params.stateByVideoId.get(videoId) || null
        ...
        const previousFingerprint = normalizeNamespaceVideoSourceFingerprint(currentRow?.source_fingerprint)
        if (previousFingerprint === sourceFingerprint) continue
        const nextRow: NamespaceVideoStateRow = {
            ...(currentRow || {}),
            namespace_id: namespaceId,
            video_id: videoId,
            source_fingerprint: sourceFingerprint,
        }
        params.stateByVideoId.set(videoId, nextRow)
...
async function seedNamespaceVideoStateFromLegacy(db: D1Database, namespaceId: string): Promise<void> {
    await ensureNamespaceVideoStateTable(db)
    const existing = await db.prepare('SELECT COUNT(*) AS total FROM namespace_video_state WHERE namespace_id = ?').bind(namespaceId).first() as { total?: number } | null
    if (Number(existing?.total || 0) > 0) return

    const legacyRows = await db.prepare(
        `SELECT namespace_id, video_id, shopee_link, lazada_link, shopee_original_link, lazada_original_link,
                shopee_converted_at, lazada_converted_at, lazada_member_id
         FROM gallery_index
         WHERE namespace_id = ? AND (TRIM(shopee_converted_at) <> '' OR TRIM(lazada_converted_at) <> '')`
    ).bind(namespaceId).all() as { results?: NamespaceVideoStateRow[] }

    for (const row of legacyRows.results || []) {
        const videoId = String(row.video_id || '').trim()
        if (!videoId) continue
        await upsertNamespaceVideoState(db, namespaceId, videoId, row)
    }
}
...
async function getNamespaceGalleryInventory(env: Env, namespaceId: string): Promise<{
    ...
    const stateByVideoId = new Map(
        stateRows
            .map((row) => [String(row.video_id || '').trim(), row] as const)
            .filter(([videoId]) => !!videoId)
    )

    const namespaceSourceVideos = sourceVideos
        .filter((video) => String((video as Record<string, unknown>).namespace_id || '').trim() === normalizedNamespaceId)

    await hydrateNamespaceVideoSourceFingerprints({
        env,
    ...
    const inventoryVideos = dedupeSystemGalleryVideos(namespaceSourceVideos)
        .map((video) => {
            ...
            const merged = {
                ...
                sourceNamespaceId: String(source.namespace_id || '').trim() || normalizedNamespaceId,
    ...
    return {
        sourceTotal: namespaceSourceVideos.length,
        videos: dedupeNamespaceInventoryVideos(inventoryVideos),
    }
}
...
async function issueAdminAccessToken(bucket: R2Bucket, chatId?: string): Promise<string> {
    const token = 'adm_' + crypto.randomUUID().replace(/-/g, '')
    await bucket.put(`_admin_session/${token}.json`, JSON.stringify({
        chat_id: String(chatId || '').trim(),
        expires_at: Date.now() + ADMIN_AUTH_TTL_MS,
    }), {
        httpMetadata: { contentType: 'application/json' },
    })
    return token
...
async function validateAdminAccessToken(bucket: R2Bucket, rawToken: string): Promise<boolean> {
    const token = String(rawToken || '').trim()
    if (!token) return false
    const sessionObj = await bucket.get(`_admin_session/${token}.json`).catch(() => null)
    if (!sessionObj) return false

    try {
        const payload = await sessionObj.json() as { expires_at?: number }
        const expiresAt = Number(payload?.expires_at || 0)
        if (!expiresAt || Date.now() > expiresAt) {
            await bucket.delete(`_admin_session/${token}.json`).catch(() => { })
            return false
        }
        return true
    } catch {
...
async function issueAdminAccessTokenForGrant(env: Env, chatId: string): Promise<string | null> {
    const normalizedChatId = String(chatId || '').trim()
    if (!normalizedChatId) return null

    const grantKey = `_admin_grant/${normalizedChatId}.json`
    const grantObj = await env.BUCKET.get(grantKey)
    if (!grantObj) return null

    try {
        const grant = await grantObj.json() as { expires_at?: number }
        const expiresAt = Number(grant?.expires_at || 0)
        if (!expiresAt || Date.now() > expiresAt) {
            await env.BUCKET.delete(grantKey).catch(() => { })
            return null
        }
    } catch {
        await env.BUCKET.delete(grantKey).catch(() => { })
    ...
    await env.BUCKET.put(grantKey, JSON.stringify({ expires_at: Date.now() + ADMIN_AUTH_TTL_MS }), {
        httpMetadata: { contentType: 'application/json' },
    }).catch(() => { })

    return issueAdminAccessToken(env.BUCKET, normalizedChatId)
}

async function issueAdminAccessTokenForLaunch(env: Env, launchTokenRaw: string): Promise<string | null> {
    const launchToken = String(launchTokenRaw || '').trim()
    if (!launchToken) return null

    const launchKey = `_admin_launch/${launchToken}.json`
    const launchObj = await env.BUCKET.get(launchKey)
    if (!launchObj) return null

    try {
        const payload = await launchObj.json() as { chat_id?: string; expires_at?: number }
        const expiresAt = Number(payload?.expires_at || 0)
        if (!expiresAt || Date.now() > expiresAt) {
            await env.BUCKET.delete(launchKey).catch(() => { })
            return null
        }

        await env.BUCKET.delete(launchKey).catch(() => { })
        return issueAdminAccessToken(env.BUCKET, String(payload?.chat_id || '').trim())
    } catch {
        await env.BUCKET.delete(launchKey).catch(() => { })
        return null
    }
...
async function generateThumbnailViaContainer(
    env: Env,
    videoUrl: string,
    frameSeed?: string,
    seekSeconds?: number,
    ...
    const resp = await containerStub.fetch('http://container/thumbnail', {
        ...
        body: JSON.stringify({
            video_url: videoUrl,
            frame_seed: String(frameSeed || '').trim(),
            output_format: outputFormat === 'jpg' ? 'jpg' : 'webp',
...
function isLikelyInvalidStoredVideoObject(obj: Pick<R2Object, 'size' | 'httpMetadata'> | null | undefined): boolean {
    if (!obj) return true
...
async function storeThumbnailForSourceUrl(params: {
    ...
    const defaultFrameSeed = `${normalizedNamespaceId}:${normalizedVideoId}:${target}`
    const thumbBytes = await generateThumbnailViaContainer(
        params.env,
        normalizedSourceUrl,
        String(params.frameSeed || '').trim() || defaultFrameSeed,
    )
...
async function materializeOriginalVideoAsset(params: {
    ...
    const hasUsableOriginal = !isLikelyInvalidStoredVideoObject(currentOriginal)
    ...
    const thumbnailUrl = await storeThumbnailForSourceUrl({
        ...
        target: 'original',
        frameSeed: `${normalizedNamespaceId}:${normalizedVideoId}:original`,
...
async function syncOriginalThumbnailToProcessedThumbnail(params: {
    ...
    namespaceId: string
    videoId: string
}): Promise<{ synced: boolean; thumbnailUrl: string }> {
    const normalizedNamespaceId = String(params.namespaceId || '').trim()
    const normalizedVideoId = String(params.videoId || '').trim()
    ...
    const processedThumbKey = getProcessedThumbnailAssetKey(normalizedVideoId)
    const originalThumbObj = await params.bucket.get(originalThumbKey).catch(() => null)
    if (!originalThumbObj) {
        return { synced: false, thumbnailUrl: '' }
    }

    const thumbBytes = await originalThumbObj.arrayBuffer().catch(() => null)
    ...
    const contentType = String(originalThumbObj.httpMetadata?.contentType || 'image/webp').trim() || 'image/webp'
    await params.bucket.put(processedThumbKey, thumbBytes, {
        httpMetadata: {
            contentType,
        },
    })
...
async function backfillGalleryThumbnail(env: Env, namespaceId: string, videoId: string): Promise<{ generated: boolean; thumbnailUrl: string }> {
    ...
    const [originalObj, publicObj, currentThumbObj, originalThumbObj] = await Promise.all([
    ...
    if (originalThumbObj) {
        const synced = await syncOriginalThumbnailToProcessedThumbnail({
            env,
            bucket,
            namespaceId: normalizedNamespaceId,
            videoId: normalizedVideoId,
        }).catch(() => ({ synced: false, thumbnailUrl: '' }))
        if (synced.synced) {
            return { generated: false, thumbnailUrl: synced.thumbnailUrl || thumbnailUrl }
        }
    }
    if (currentThumbObj) {
    ...
    const storedThumbnailUrl = await storeThumbnailForSourceUrl({
        ...
        target: 'processed',
        frameSeed: `${normalizedNamespaceId}:${normalizedVideoId}:processed`,
    })
...
app.get('/admin', async (c) => {
    ...
    if (launchToken) {
        const adminToken = await issueAdminAccessTokenForLaunch(c.env, launchToken)
        if (adminToken) {
...
const adminAuthMiddleware = async (c: any, next: any) => {
    ...
    const tokenValue = String(adminToken || '').trim()
    if (setting?.value && tokenValue === String(setting.value || '').trim()) {
        await next()
        return
    }
    if (!await validateAdminAccessToken(c.env.BUCKET, tokenValue)) return c.json({ error: 'Unauthorized' }, 401)
    await next()
}
...
app.post('/admin/api/auto-auth', async (c) => {
    ...
    const adminToken = await issueAdminAccessTokenForGrant(c.env, chatId)
    if (!adminToken) return c.json({ error: 'Unauthorized' }, 401)
...
app.post('/admin/api/migrate-namespaces', async (c) => {
    ...
    // Collect namespace IDs from every workspace mapping table so legacy orphan rows
    // in users/line_users are migrated too.
    const namespaceIds = new Set<string>()
    const namespaceSources = [
        'SELECT DISTINCT namespace_id FROM email_namespaces',
        'SELECT DISTINCT namespace_id FROM users',
        'SELECT DISTINCT namespace_id FROM line_users',
    ]
    for (const sql of namespaceSources) {
        try {
            const rows = await db.prepare(sql).all() as { results?: Array<{ namespace_id?: string }> }
            for (const row of rows.results || []) {
                const nsId = String(row?.namespace_id || '').trim()
                if (nsId) namespaceIds.add(nsId)
            }
        } catch {
            // Skip missing tables in older DB states.
        }
    }

    const toMigrate: Array<{ oldId: string; newId: string }> = []
    for (const nsId of namespaceIds) {
        if (!nsId || CANONICAL_WORKSPACE_ID_PATTERN.test(nsId)) continue
        const newId = createOwnerWorkspaceId()
        toMigrate.push({ oldId: nsId, newId })
    }

    if (toMigrate.length === 0) {
        return c.json({ ok: true, message: 'No namespaces need migration', migrated: [] })
    }

    const results: Array<{ oldId: string; newId: string; tables: Record<string, number>; r2_copied: number }> = []

    for (const { oldId, newId } of toMigrate) {
        const tables: Record<string, number> = {}

        // Update all DB tables
        const updates: Array<{ table: string; sql: string }> = [
            { table: 'email_namespaces', sql: 'UPDATE email_namespaces SET namespace_id = ? WHERE namespace_id = ?' },
            { table: 'users', sql: 'UPDATE users SET namespace_id = ? WHERE namespace_id = ?' },
            { table: 'line_users', sql: 'UPDATE line_users SET namespace_id = ? WHERE namespace_id = ?' },
            { table: 'team_members', sql: 'UPDATE team_members SET owner_namespace_id = ? WHERE owner_namespace_id = ?' },
            { table: 'namespace_settings', sql: 'UPDATE namespace_settings SET namespace_id = ? WHERE namespace_id = ?' },
            { table: 'channels', sql: 'UPDATE channels SET bot_id = ? WHERE bot_id = ?' },
            { table: 'pages', sql: 'UPDATE pages SET bot_id = ? WHERE bot_id = ?' },
            { table: 'post_queue', sql: 'UPDATE post_queue SET bot_id = ? WHERE bot_id = ?' },
            { table: 'post_history', sql: 'UPDATE post_history SET bot_id = ? WHERE bot_id = ?' },
            { table: 'gallery_index', sql: 'UPDATE gallery_index SET namespace_id = ? WHERE namespace_id = ?' },
            { table: 'namespace_video_state', sql: 'UPDATE namespace_video_state SET namespace_id = ? WHERE namespace_id = ?' },
            { table: 'link_submissions', sql: 'UPDATE link_submissions SET namespace_id = ? WHERE namespace_id = ?' },
            { table: 'imported_videos(namespace)', sql: 'UPDATE imported_videos SET namespace_id = ? WHERE namespace_id = ?' },
            { table: 'imported_videos(source)', sql: 'UPDATE imported_videos SET source_namespace_id = ? WHERE source_namespace_id = ?' },
            { table: 'telegram_bot_sessions(namespace)', sql: 'UPDATE telegram_bot_sessions SET namespace_id = ? WHERE namespace_id = ?' },
            { table: 'telegram_bot_sessions(scope)', sql: 'UPDATE telegram_bot_sessions SET bot_scope = ? WHERE bot_scope = ?' },
        ]

        for (const { table, sql } of updates) {
            try {
                const result = await db.prepare(sql).bind(newId, oldId).run()
                tables[table] = Number(result?.meta?.changes || 0)
            } catch (e) {
                tables[table + '_error'] = 0
            }
        }

        // Migrate R2 objects: copy from old prefix to new prefix
        let r2Copied = 0
        let cursor: string | undefined
        const oldKeys: string[] = []
        while (true) {
            const listRes = await bucket.list({ prefix: `${oldId}/`, cursor, limit: 100 })
            for (const obj of listRes.objects || []) {
                const oldKey = obj.key
                const newKey = `${newId}/${oldKey.substring(oldId.length + 1)}`
                try {
                    const data = await bucket.get(oldKey)
                    if (data) {
                        await bucket.put(newKey, data.body, {
        ...
        // Delete old R2 objects after successful copy
        if (oldKeys.length > 0) {
            for (let i = 0; i < oldKeys.length; i += 100) {
                const batch = oldKeys.slice(i, i + 100)
                await bucket.delete(batch).catch(() => { })
            }
        }

        results.push({ oldId, newId, tables, r2_copied: r2Copied })
    }

    await bucket.delete('_admin_cache/all_gallery_videos.json').catch(() => { })
    await bucket.delete('_admin_cache/all_gallery_owner_videos.json').catch(() => { })

    return c.json({ ok: true, migrated: results })
})
...
const CANONICAL_WORKSPACE_ID_PATTERN = /^\d{19}$/
...
app.post('/admin/api/emails', async (c) => {
    ...
    const existingNamespace = await c.env.DB.prepare(
        'SELECT namespace_id FROM email_namespaces WHERE email = ? LIMIT 1'
    ).bind(emailLower).first() as { namespace_id?: string } | null
    ...
    const isNew = !existingNamespace?.namespace_id
    ...
    try {
        ...
        // Keep allowed_emails in sync for backward compatibility during transition
        await c.env.DB.prepare('INSERT OR IGNORE INTO allowed_emails (email) VALUES (?)').bind(emailLower).run().catch(() => { })
...
app.put('/admin/api/emails/:email/password', async (c) => {
    ...
    const owner = await c.env.DB.prepare(
        'SELECT email FROM email_namespaces WHERE email = ? LIMIT 1'
...
app.delete('/admin/api/users/:email', async (c) => {
    ...
    if (!namespaceId) {
        ...
        await c.env.DB.prepare('DELETE FROM allowed_emails WHERE email = ?').bind(email).run().catch(() => { })
        await c.env.DB.prepare('DELETE FROM team_members WHERE lower(trim(email)) = ?').bind(email).run()
    ...
    deleted.namespace_settings = toChanges(await c.env.DB.prepare('DELETE FROM namespace_settings WHERE namespace_id = ?').bind(namespaceId).run().catch(() => ({ meta: { changes: 0 } })))
    ...
    deleted.imported_videos_namespace = toChanges(await c.env.DB.prepare('DELETE FROM imported_videos WHERE namespace_id = ?').bind(namespaceId).run().catch(() => ({ meta: { changes: 0 } })))
    deleted.imported_videos_source = toChanges(await c.env.DB.prepare('DELETE FROM imported_videos WHERE source_namespace_id = ?').bind(namespaceId).run().catch(() => ({ meta: { changes: 0 } })))
    deleted.telegram_sessions_namespace = toChanges(await c.env.DB.prepare('DELETE FROM telegram_bot_sessions WHERE namespace_id = ?').bind(namespaceId).run().catch(() => ({ meta: { changes: 0 } })))
    deleted.telegram_sessions_scope = toChanges(await c.env.DB.prepare('DELETE FROM telegram_bot_sessions WHERE bot_scope = ?').bind(namespaceId).run().catch(() => ({ meta: { changes: 0 } })))
    deleted.line_users = toChanges(await c.env.DB.prepare('DELETE FROM line_users WHERE namespace_id = ?').bind(namespaceId).run().catch(() => ({ meta: { changes: 0 } })))
    ...
    let removedOwnerEmails = 0
    for (const ownerEmail of ownerEmails) {
        removedOwnerEmails += toChanges(await c.env.DB.prepare('DELETE FROM allowed_emails WHERE email = ?').bind(ownerEmail).run().catch(() => ({ meta: { changes: 0 } })))
        removedOwnerEmails += toChanges(await c.env.DB.prepare('DELETE FROM email_namespaces WHERE email = ?').bind(ownerEmail).run().catch(() => ({ meta: { changes: 0 } })))
    }
    deleted.owner_emails = removedOwnerEmails
...
app.post('/api/auth/login', async (c) => {
    ...
    if (tg_id && !email) {
        if (botScope) {
            ...
            const existingSessionToken = String(c.req.header('x-auth-token') || '').trim()
            const bound = await bindTelegramToExistingSession(c.env.DB, existingSessionToken, String(tg_id), botScope)
            if (bound?.session_token) {
                return c.json({ session_token: bound.session_token, rebound: true })
            }
            const provisioned = await provisionTelegramOwnerSession(c.env.DB, String(tg_id), botScope)
            if (provisioned?.session_token) {
                return c.json({ session_token: provisioned.session_token, is_owner: true, auto_provisioned: true })
            }
            return c.json({ error: 'Unauthorized' }, 401)
        }
        const user = await c.env.DB.prepare(
    ...
    // Check if team member first, otherwise treat as namespace owner
    const teamMember = await c.env.DB.prepare('SELECT owner_namespace_id FROM team_members WHERE lower(trim(email)) = ?').bind(emailLower).first() as any
    ...
    // - Owner (anyone not a team member): ensure namespace via email_namespaces
    let namespaceId: string | null = null

    if (teamMember?.owner_namespace_id) {
        namespaceId = String(teamMember.owner_namespace_id)
    } else {
        // Every non-team-member is a namespace owner - ensure they have a proper namespace
        namespaceId = await ensureOwnerWorkspaceUsesEmailNamespace(c.env.DB, emailLower, { preferExistingNamespace: true })
    }

    // Persist canonical email mapping for all namespace owners
    if (isOwner && namespaceId) {
        try {
    ...
    if (bindTgId && botScope) {
        await upsertUserSessionByTelegram(c.env.DB, bindTgId, emailLower, namespaceId, sessionToken)
        await upsertTelegramBotSession(c.env.DB, bindTgId, botScope, emailLower, namespaceId, sessionToken)
    } else if (bindTgId) {
        await upsertUserSessionByTelegram(c.env.DB, bindTgId, emailLower, namespaceId, sessionToken)
    } else {
        // Reuse the bot-owner row when present so one owner email maps to one account row.
        const existingEmailUser = await c.env.DB.prepare(
            `SELECT u.telegram_id
             FROM users u
             LEFT JOIN channels ch
               ON CAST(u.telegram_id AS TEXT) = CAST(ch.owner_telegram_id AS TEXT)
             WHERE lower(trim(u.email)) = ?
             ORDER BY
               CASE WHEN ch.owner_telegram_id IS NOT NULL THEN 0 ELSE 1 END ASC,
               datetime(u.created_at) ASC,
               u.telegram_id ASC
             LIMIT 1`
        ).bind(emailLower).first() as { telegram_id?: number } | null

        if (existingEmailUser?.telegram_id != null) {
            await c.env.DB.prepare(
                'UPDATE users SET email = ?, namespace_id = ?, session_token = ? WHERE telegram_id = ?'
            ).bind(emailLower, namespaceId, sessionToken, Number(existingEmailUser.telegram_id)).run()
            await c.env.DB.prepare(
                `UPDATE telegram_bot_sessions
                 SET email = ?, namespace_id = ?, session_token = ?, updated_at = datetime('now')
                 WHERE telegram_id = ?`
            ).bind(emailLower, namespaceId, sessionToken, String(existingEmailUser.telegram_id)).run().catch(() => { })

            if (isOwner) {
                await c.env.DB.prepare(
                    'DELETE FROM users WHERE lower(trim(email)) = ? AND telegram_id <> ?'
                ).bind(emailLower, Number(existingEmailUser.telegram_id)).run()
            }
        } else {
    ...
    const sysAdmin = isOwner ? await isSystemAdmin(c.env.DB, emailLower) : false
    return c.json({ session_token: sessionToken, is_owner: isOwner, is_system_admin: sysAdmin })
})
...
app.post('/api/auth/resolve-email', async (c) => {
    ...
    ).bind(email).first() as { owner_namespace_id?: string } | null

    if (team?.owner_namespace_id) {
        // Team member: resolve via owner namespace
        ...
        const teamNamespaces = Array.from(new Set(
            (teamRows.results || []).map(r => String(r.owner_namespace_id || '').trim()).filter(Boolean)
        ))
        return c.json({
            success: true,
            namespace_id: teamNamespaces[0],
    ...
    // Namespace owner (anyone not a team member)
...
app.get('/api/team', async (c) => {
    ...
    const { results } = await c.env.DB.prepare(
        `SELECT t.email, t.created_at,
                ...
                COALESCE(l.picture_url, '') as picture_url,
                COALESCE(l.line_user_id, '') as line_user_id
         FROM team_members t
         LEFT JOIN line_users l ON lower(trim(l.email)) = lower(trim(t.email))
...
app.delete('/api/team/:email', async (c) => {
    ...
    const normalizedEmail = String(email || '').trim().toLowerCase()
    await c.env.DB.prepare("UPDATE users SET session_token = '' WHERE lower(trim(email)) = ? AND namespace_id = ?")
        .bind(normalizedEmail, namespaceId).run()
    await c.env.DB.prepare('DELETE FROM team_members WHERE lower(trim(email)) = ? AND owner_namespace_id = ?')
        .bind(normalizedEmail, namespaceId).run()
    return c.json({ ok: true })
})
...
app.get('/api/me', async (c) => {
    ...
    const lineUser = await c.env.DB.prepare(
        `SELECT line_user_id, display_name, picture_url
         ...
         ORDER BY datetime(updated_at) DESC, datetime(created_at) DESC
         LIMIT 1`
    ).bind(String(user.email || '').trim().toLowerCase()).first().catch(() => null) as any
        || await c.env.DB.prepare(
            `SELECT line_user_id, display_name, picture_url
             FROM line_users
             WHERE namespace_id = ?
    ...
    const teamRow = await c.env.DB.prepare(
        'SELECT owner_namespace_id FROM team_members WHERE lower(trim(email)) = ? LIMIT 1'
    ).bind(String(user.email || '').trim().toLowerCase()).first().catch(() => null) as { owner_namespace_id?: string } | null
    const isTeamMember = !!teamRow && String(teamRow.owner_namespace_id || '').trim() === String(user.namespace_id || '').trim()
...
app.get('/api/settings/voice-prompt', async (c) => {
    ...
    const namespaceId = c.get('botId')
    const settings = await getNamespaceVoiceSettings(c.env.DB, namespaceId)
    return c.json({
        ...
        voice_options: GEMINI_TTS_VOICE_OPTIONS,
...
app.put('/api/settings/voice-prompt', async (c) => {
    ...
    const latest = await getNamespaceVoiceSettings(c.env.DB, namespaceId)
    return c.json({
        ok: true,
        profile: latest.profile,
        prompt: latest.scriptPrompt,
...
app.delete('/api/settings/voice-prompt', async (c) => {
    const ownerCheck = await requireAuthSession(c)
    if (!ownerCheck.ok) return ownerCheck.response

    const namespaceId = c.get('botId')
    const latest = await clearNamespaceVoiceSettings(c.env.DB, namespaceId)
    return c.json({
        ok: true,
        profile: latest.profile,
        prompt: latest.scriptPrompt,
...
app.post('/api/settings/voice-preview', async (c) => {
    const ownerCheck = await requireAuthSession(c)
    if (!ownerCheck.ok) return ownerCheck.response
        ...
        try {
            ...
            return new Response(wav, {
                ...
                headers: {
                    ...
                    'Cache-Control': 'no-store',
                    'x-voice-name': profile.voice_name,
                },
            })
        } catch (error) {
            lastError = error instanceof Error ? error.message : String(error)
...
app.get('/api/settings/comment-template', async (c) => {
    ...
    return c.json({
        ...settings,
        max_chars: MAX_COMMENT_TEMPLATE_CHARS,
        placeholders: {
            shopee_link: COMMENT_TEMPLATE_SHOPEE_PLACEHOLDER,
            lazada_link: COMMENT_TEMPLATE_LAZADA_PLACEHOLDER,
        },
    })
})
...
app.get('/api/settings/cover-template', async (c) => {
    const ownerCheck = await requireAuthSession(c)
    if (!ownerCheck.ok) return ownerCheck.response
...
app.put('/api/settings/cover-template', async (c) => {
    const ownerCheck = await requireAuthSession(c)
    if (!ownerCheck.ok) return ownerCheck.response
...
app.put('/api/settings/gemini-key', async (c) => {
    ...
    const body = await c.req.json().catch(() => ({})) as { api_key?: string; apiKey?: string; api_keys?: string[]; apiKeys?: string[]; preserve_existing?: boolean }
    const rawIncomingKeys = Array.isArray(body.api_keys)
        ? body.api_keys
        : Array.isArray(body.apiKeys)
            ? body.apiKeys
            : [String(body.api_key ?? body.apiKey ?? '').trim()]
    const preserveExisting = body.preserve_existing !== false

    for (const raw of rawIncomingKeys) {
        const key = String(raw || '').trim()
        if (key.length > MAX_GEMINI_API_KEY_CHARS) {
...
app.delete('/api/settings/gemini-key', async (c) => {
    ...
    const namespaceId = c.get('botId')
    await setSystemGeminiApiKeys(c.env.DB, [])
    const settings = await getNamespaceGeminiApiKeySettings(c.env.DB, namespaceId)
    return c.json({
        ok: true,
        ...settings,
...
function buildLineCompactFlexCard(params: {
    altText?: string
    pillText: string
    title: string
    message: string
    ...
    const detailLabel = String(params.detailLabel || '').trim()
    const detailText = String(params.detailText || '').trim()
    const actionLabel = String(params.actionLabel || '').trim()
    const actionUrl = String(params.actionUrl || '').trim()

    return {
        type: 'flex',
        altText: params.altText || `${params.title} ${params.message}`,
        contents: {
            type: 'bubble',
            size: 'mega',
            body: {
                ...
                contents: [
                                ...
                                contents: [
                                    {
                                        ...
                                        contents: [{
                                            type: 'text',
                                            text: params.pillText,
                                            size: 'xs',
                                            weight: 'bold',
                                            color: params.primaryDark,
                                            align: 'center',
                                        }],
                                    },
                                ],
                    ...
                    ...(detailLabel && detailText ? [{
                        ...
                        paddingAll: '14px',
                        cornerRadius: '16px',
                        backgroundColor: params.surface,
                        contents: [
                            {
                                type: 'text',
                                text: detailLabel,
...
function buildLineAffiliatePromptFlex(params: {
    ...
    const message = params.message || `ส่งลิงก์ ${theme.brand} มาเลย`
    return buildLineCompactFlexCard({
        altText: `${params.title} ${message}`,
        pillText: `${theme.icon} ${theme.brand}`,
        title: params.title,
        message,
...
function buildLineCoverOptionSeed(videoId: string, optionIndex: number, optionCount: number, batchKey?: string): string {
    const normalizedVideoId = String(videoId || '').trim()
    const normalizedBatchKey = String(batchKey || 'base').trim() || 'base'
    const normalizedOptionIndex = Math.max(1, Math.floor(Number(optionIndex) || 0) || 1)
    const normalizedOptionCount = Math.max(normalizedOptionIndex, Math.floor(Number(optionCount) || 0) || 1)
    return `line-cover|${normalizedVideoId}|${normalizedBatchKey}|${normalizedOptionIndex}|${normalizedOptionCount}`
}

function createLineCoverOptionBatchKey(): string {
    return `${Date.now().toString(36)}${Math.random().toString(36).slice(2, 6)}`
}

function buildLineCoverOptions(videoId: string, count = 10, batchKey?: string): LineCoverOption[] {
    const normalizedVideoId = String(videoId || '').trim()
    const optionCount = Math.max(1, Math.min(10, Math.floor(Number(count) || 0) || 10))
    const normalizedBatchKey = String(batchKey || createLineCoverOptionBatchKey()).trim() || 'base'
    return Array.from({ length: optionCount }, (_, index) => ({
        optionId: String(index + 1),
        seed: buildLineCoverOptionSeed(normalizedVideoId, index + 1, optionCount, normalizedBatchKey),
    }))
}

function buildLineCoverSelectionPostbackData(videoId: string, optionId: string): string {
    return `cover_select:${String(videoId || '').trim()}:${String(optionId || '').trim()}`
}

function parseLineCoverSelectionPostbackData(input: unknown): { videoId: string; optionId: string } | null {
    const normalized = String(input || '').trim()
    const match = normalized.match(/^cover_select:([A-Za-z0-9_-]+):([A-Za-z0-9_-]+)$/)
    if (!match) return null
    return {
...
function buildLineCoverOptionPreviewUrl(params: {
    ...
    const workerUrl = String(params.workerUrl || '').trim().replace(/\/+$/, '')
    const namespaceId = String(params.namespaceId || '').trim()
    const videoId = String(params.videoId || '').trim()
...
function buildLineCoverPickerFlex(params: {
    ...
    const coverOptions = normalizeLineCoverOptions(params.coverOptions)
    return {
        type: 'flex',
        altText: 'เลือกหน้าปกได้เลย',
        quickReply: { items: LINE_COVER_PICKER_QUICK_REPLY_ITEMS },
        contents: {
            type: 'carousel',
                        ...
                        contents: [
                            {
                                ...
                                action: {
                                    type: 'postback',
                                    label: `เลือกปก ${optionId}`,
                                    data: postbackData,
                                    displayText: `เลือกปก ${optionId}`,
                                },
                            },
                            {
                                type: 'box',
...
function buildLineChoicePromptFlexCard(params: {
    altText?: string
    pillText: string
    title: string
    message: string
                ...
                contents: [
                    {
                        ...
                        contents: [
                            ...
                            },
                            {
                                type: 'text',
                                text: params.message,
                                color: params.messageColor || '#FFF7F5',
                                size: 'sm',
                                wrap: true,
                            },
                        ],
                ...
                contents: [
                    {
                        ...
                        action: {
                            type: 'message',
                            label: params.primaryActionLabel,
                            text: params.primaryActionText,
                        },
                        style: 'primary',
                        color: params.primary,
                        height: 'sm',
                    },
                    {
                        ...
                        action: {
                            type: 'message',
                            label: params.secondaryActionLabel,
                            text: params.secondaryActionText,
                        },
                        style: 'secondary',
                        height: 'sm',
                    },
...
function buildLineCaptionPromptFlex(params?: {
    ...
    const title = String(params?.title || '').trim() || 'รับลิงก์ครบแล้ว'
    const message = String(params?.message || '').trim() || 'พิมพ์แคปชั่นมาได้เลย'
    return buildLineChoicePromptFlexCard({
        ...
        primaryDark: '#5B21B6',
        messageColor: '#F5F3FF',
        primaryActionLabel: 'แก้ไขแคปชั่น',
        primaryActionText: 'กดแก้ไขแคปชั่น',
        secondaryActionLabel: 'ให้ AI คิดให้',
        secondaryActionText: 'ข้าม ใช้ AI',
        secondarySurface: '#DDE2EC',
        secondaryTextColor: '#111827',
    })
}
...
function buildLineProcessingReadyFlex(params: {
    ...
    const primary = processed ? '#0F766E' : '#DC2626'
    const primaryDark = processed ? '#115E59' : '#991B1B'
    const surface = processed ? '#ECFDF5' : '#FEF2F2'
    const statusText = String(params.statusLabel || '').trim()
    const videoId = String(params.videoId || '').trim()
    return buildLineCompactFlexCard({
        altText: `${params.title} ${params.message}`,
        pillText: processed ? '✅ Gallery' : `⚙️ ${statusText || 'Processing'}`,
        title: params.title,
        message: params.message,
        primary,
        primaryDark,
        surface,
        detailLabel: videoId ? 'Video ID' : '',
        detailText: videoId,
...
function buildLineCoverProcessingFlex(params: {
    previewUrl: string
    title: string
    message: string
    actionLabel: string
...
async function resolveLineCoverPreviewUrl(params: {
    env: Env
    bucket: R2Bucket
    namespaceId: string
    videoId: string
    ...
    const jpgKey = `_inbox_cover/${videoId}_preview.jpg`
    const webpKey = `_inbox_cover/${videoId}.webp`
    const jpgObj = await params.bucket.head(jpgKey).catch(() => null)
    if (jpgObj) return `${params.env.WORKER_URL}/api/inbox-cover/${encodeURIComponent(videoId)}/preview?namespace_id=${encodeURIComponent(namespaceId)}`
    const webpObj = await params.bucket.head(webpKey).catch(() => null)
    if (webpObj) return `${params.env.WORKER_URL}/api/inbox-cover/${encodeURIComponent(videoId)}/preview?namespace_id=${encodeURIComponent(namespaceId)}`
    return ''
}
...
async function lineReply(
    ...
    const enriched = messages.map((msg, i) => {
        if (i === messages.length - 1) {
            const existingQuickReply = (msg as Record<string, unknown>)?.quickReply as { items?: unknown } | undefined
            const existingItems = Array.isArray(existingQuickReply?.items)
    ...
    if (!resp.ok) {
        ...
        throw new Error(`line_reply_failed_${resp.status}${errText ? `: ${errText}` : ''}`)
    }
}
...
async function lineStartLoading(
    ...
    const seconds = Math.max(5, Math.min(60, Math.round(Number(loadingSeconds) || 0)))
    if (!seconds) return
    ...
    try {
        ...
        if (!resp.ok) {
            const errText = await resp.text().catch(() => '')
            console.warn(`[LINE] loading/start failed userId=${userId} status=${resp.status} body=${errText.slice(0, 200)}`)
        }
    } catch (e) {
        console.warn(`[LINE] loading/start error userId=${userId}: ${e instanceof Error ? e.message : String(e)}`)
    }
}

function normalizeManualCaption(input: unknown): string {
    const normalized = String(input || '')
        .replace(/\r\n/g, '\n')
        .replace(/\u00a0/g, ' ')
        .trim()
    if (!normalized) return ''
    return normalized.slice(0, MAX_LINE_MANUAL_CAPTION_CHARS)
}

function normalizeLineCoverOptions(input: unknown): LineCoverOption[] {
    if (!Array.isArray(input)) return []
...
function normalizeLineWaitingVideoState(input: Partial<LineWaitingVideoState> | null | undefined): LineWaitingVideoState | null {
...
async function getLineWaitingVideoState(bucket: R2Bucket, lineUserId: string): Promise<LineWaitingVideoState | null> {
    ...
    const raw = await waitingObj.json().catch(() => null) as Partial<LineWaitingVideoState> | null
...
async function putLineWaitingVideoState(bucket: R2Bucket, lineUserId: string, input: Partial<LineWaitingVideoState>): Promise<LineWaitingVideoState> {
    const state = normalizeLineWaitingVideoState(input)
    if (!state) throw new Error('invalid_line_waiting_state')
    await bucket.put(`_waiting_video/${lineUserId}.json`, JSON.stringify(state), {
        httpMetadata: { contentType: 'application/json' },
    })
    return state
}

async function clearLineWaitingVideoState(bucket: R2Bucket, lineUserId: string): Promise<void> {
    await bucket.delete(`_waiting_video/${lineUserId}.json`).catch(() => { })
}
...
async function listReadyLineWaitingVideoStates(bucket: R2Bucket): Promise<LineWaitingVideoState[]> {
    ...
    const tasks = await Promise.all(list.objects.map(async (obj) => {
        const file = await bucket.get(obj.key).catch(() => null)
        if (!file) return null
        const raw = await file.json().catch(() => null) as Partial<LineWaitingVideoState> | null
        const state = normalizeLineWaitingVideoState(raw)
        if (!state) return null
        if (!state.shopeeLink || !state.lazadaLink) return null
        return state
    }))
    return tasks
        .filter((item): item is LineWaitingVideoState => !!item)
        .sort((a, b) => new Date(String(b.createdAt || '')).getTime() - new Date(String(a.createdAt || '')).getTime())
}

function isLineCaptionSkipCommand(input: string): boolean {
    const normalized = String(input || '').trim().toLowerCase()
    return normalized === 'ข้าม'
        || normalized === 'skip'
        || normalized === 'ข้าม ใช้ ai'
        || normalized === 'ข้ามใช้ ai'
}
...
function isLineCoverSkipCommand(input: string): boolean {
    const normalized = String(input || '').trim().toLowerCase()
    return normalized === 'ข้ามเลือกปก'
        || normalized === 'ข้าม'
        || normalized === 'skip'
        || normalized === 'เสร็จแล้ว'
        || normalized === 'done'
        || normalized === 'ส่งเลย'
}

function isLineCoverReshuffleCommand(input: string): boolean {
    const normalized = String(input || '').trim().toLowerCase()
    return normalized === 'สุ่มอีกครั้ง'
        || normalized === 'สุ่มใหม่'
        || normalized === 'สุ่มปกใหม่'
        || normalized === 'shuffle again'
...
function stripLineAffiliateLinksFromText(input: string): string {
    return String(input || '')
        ...
        .replace(/https?:\/\/shope\.ee\S+/gi, ' ')
        .replace(/https?:\/\/\S*(?:lazada|lzd\.co)\S+/gi, ' ')
        .replace(/[ \t]+/g, ' ')
        .replace(/\n{3,}/g, '\n\n')
        .trim()
}

async function promptLineCaptionCollection(params: {
    env: Env
    bucket: R2Bucket
    namespaceId: string
    channelAccessToken: string
    ...
    await syncWaitingVideoIntoPrimaryAdminNamespace({
        env: params.env,
...
async function materializeSelectedLineCoverOptionAssets(params: {
    ...
    if (!namespaceId || !videoId || !option?.seed) throw new Error('invalid_line_cover_option_request')
    ...
    if (jpgBytes && jpgBytes.length > 0) {
        await params.bucket.put(`_inbox_cover/${videoId}_preview.jpg`, jpgBytes, {
            httpMetadata: { contentType: 'image/jpeg' },
        })
    } else {
        await params.bucket.delete(`_inbox_cover/${videoId}_preview.jpg`).catch(() => { })
    }
}
...
async function finalizeLineWaitingVideoAndStartProcessing(params: {
    ...
    const inboxRecord = await putInboxVideoRecord(params.bucket, {
        ...
        chatId: deriveLineSubmissionChatId(params.lineUserId),
        createdAt: String(waitingState.createdAt || '').trim() || nowIso,
        sourceType: (waitingState.sourceType as InboxVideoSourceType) || 'line_video',
        sourceLabel: waitingState.sourceLabel || 'LINE video',
        shopeeLink: waitingState.shopeeLink,
        lazadaLink: waitingState.lazadaLink,
        shopeeOriginalLink: waitingState.shopeeLink,
        lazadaOriginalLink: waitingState.lazadaLink,
        manualCaption,
...
async function linePushMessage(
    ...
    const token = String(channelAccessToken || '').trim()
    const userId = String(lineUserId || '').trim()
    if (!token || !userId) return { ok: false, error: 'missing_token_or_user_id' }
    try {
...
app.post('/api/line/liff-login', async (c) => {
    ...
    const linkedRow = await c.env.DB.prepare(
        'SELECT email, status, namespace_id FROM line_users WHERE line_user_id = ?'
    ).bind(lineUserId).first().catch(() => null) as { email?: string; status?: string; namespace_id?: string } | null
    ...
    if (hasRealLink) {
        ...
        isOwner = await isNamespaceOwnerIdentity(c.env.DB, email, namespaceId)
        ...
        const syntheticSysAdmin = await isSystemAdmin(c.env.DB, email)

        if (syntheticSysAdmin) {
            const syntheticTeamMember = await isTeamMemberEmail(c.env.DB, email)
            namespaceId = await resolveManagedWorkspaceForRole(
                c.env.DB,
                email,
                String(linkedRow?.namespace_id || '').trim(),
                syntheticTeamMember ? String(linkedRow?.namespace_id || '').trim() : ''
            )
            await upsertOwnerNamespaceMapping(c.env.DB, email, namespaceId)
            await c.env.DB.prepare(
            ...
            const existingTeam = await c.env.DB.prepare(
                'SELECT owner_namespace_id FROM team_members WHERE lower(trim(email)) = ? LIMIT 1'
            ).bind(String(email || '').trim().toLowerCase()).first().catch(() => null) as { owner_namespace_id?: string } | null
            const existingOwnerNamespace = await c.env.DB.prepare(
                'SELECT namespace_id FROM email_namespaces WHERE email = ? LIMIT 1'
            ).bind(String(email || '').trim().toLowerCase()).first().catch(() => null) as { namespace_id?: string } | null
            const existingOwnerNamespaceId = String(existingOwnerNamespace?.namespace_id || '').trim()

            if (inviteNs) {
                // Invite link always wins for synthetic LINE accounts.
                // This fixes users who first entered as standalone Member and later accepted a Team invite.
                ...
                await c.env.DB.prepare(
                    `DELETE FROM team_members WHERE lower(trim(email)) = ? AND owner_namespace_id <> ?`
                ).bind(email, inviteNs).run().catch(() => { })
                // Add as team member
                await c.env.DB.prepare(
    ...
    // Team members (invited) are auto-approved, Members (self-registered) need admin approval
    const isAdminEmail = isSysAdmin || (hasRealLink && await isSystemAdmin(c.env.DB, email))
    const teamCheck = await c.env.DB.prepare('SELECT owner_namespace_id FROM team_members WHERE LOWER(email) = LOWER(?) LIMIT 1').bind(email).first().catch(() => null) as { owner_namespace_id?: string } | null
    const isTeamMember = !!teamCheck?.owner_namespace_id
    let userStatus: string
    if (isAdminEmail || isTeamMember) {
        userStatus = 'approved'
    } else if (existingStatus === 'approved') {
        userStatus = 'approved'
        // Auto-fix: ensure email_namespaces exists for approved members
        if (!isTeamMember && namespaceId) {
            await c.env.DB.prepare(
                `INSERT INTO email_namespaces (email, namespace_id) VALUES (?, ?) ON CONFLICT(email) DO NOTHING`
            ).bind(email, namespaceId).run().catch(() => { })
            isOwner = true
        }
    } else {
    ...
    // Reuse active session when possible so app/web don't invalidate each other.
    const existingUser = await c.env.DB.prepare(
        'SELECT email, session_token FROM users WHERE email = ? LIMIT 1'
    ).bind(email).first() as { email?: string; session_token?: string } | null
    const existingSessionToken = String(existingUser?.session_token || '').trim()
    const sessionToken = existingSessionToken.startsWith('sess_')
...
app.get('/api/admin/members', async (c) => {
    ...
    const { results } = await c.env.DB.prepare(
        `WITH team_roles AS (
            SELECT lower(trim(email)) AS email_key, MIN(owner_namespace_id) AS owner_namespace_id
            FROM team_members
         ...
         SELECT
            ...
            l.namespace_id,
            CASE
                WHEN sa.email IS NOT NULL THEN 'approved'
                WHEN tr.owner_namespace_id IS NOT NULL AND tr.owner_namespace_id = l.namespace_id THEN 'approved'
                ELSE l.status
            END AS status,
            l.created_at,
            l.updated_at,
            tr.owner_namespace_id AS team_owner_namespace_id,
            CASE
                WHEN sa.email IS NOT NULL THEN 'admin'
                WHEN tr.owner_namespace_id IS NOT NULL AND tr.owner_namespace_id = l.namespace_id THEN 'team'
                ELSE 'member'
            END AS role
         FROM line_users l
         LEFT JOIN team_roles tr ON tr.email_key = lower(trim(l.email))
         LEFT JOIN system_admins sa ON lower(trim(sa.email)) = lower(trim(l.email))
...
app.get('/api/admin/pending-users', async (c) => {
    ...
    const { results } = await c.env.DB.prepare(
        `WITH team_roles AS (
         ...
         SELECT l.line_user_id, l.display_name, l.picture_url, l.created_at
         FROM line_users l
         LEFT JOIN team_roles tr ON tr.email_key = lower(trim(l.email))
         LEFT JOIN system_admins sa ON lower(trim(sa.email)) = lower(trim(l.email))
         WHERE l.status = 'pending'
           AND tr.email_key IS NULL
...
app.post('/api/admin/approve-user', async (c) => {
    ...
    const approvedUser = await c.env.DB.prepare(
        'SELECT email, namespace_id FROM line_users WHERE line_user_id = ?'
    ).bind(lineUserId).first() as { email?: string; namespace_id?: string } | null
    const approvedEmail = String(approvedUser?.email || '').trim().toLowerCase()
    const approvedNamespaceId = String(approvedUser?.namespace_id || '').trim()
    const isTeam = approvedEmail
        ? await isTeamMemberEmail(c.env.DB, approvedEmail)
        : false

    if (approvedEmail && approvedNamespaceId && !isTeam) {
        await c.env.DB.prepare(
            `INSERT INTO email_namespaces (email, namespace_id) VALUES (?, ?) ON CONFLICT(email) DO UPDATE SET namespace_id = excluded.namespace_id`
        ).bind(approvedEmail, approvedNamespaceId).run().catch(() => { })
    }
    if (approvedEmail && approvedNamespaceId) {
        await upsertUserWorkspaceByEmail(c.env.DB, approvedEmail, approvedNamespaceId)
    }
...
app.post('/api/admin/reject-user', async (c) => {
    ...
    await ensureLineUsersTable(c.env.DB)
    const rejectedUser = await c.env.DB.prepare(
        'SELECT email FROM line_users WHERE line_user_id = ? LIMIT 1'
    ).bind(lineUserId).first().catch(() => null) as { email?: string } | null
    const rejectedEmail = String(rejectedUser?.email || '').trim().toLowerCase()
    await c.env.DB.prepare(
        `DELETE FROM line_users WHERE line_user_id = ?`
    ).bind(lineUserId).run()
    if (rejectedEmail) {
        await c.env.DB.prepare('DELETE FROM email_namespaces WHERE email = ?').bind(rejectedEmail).run().catch(() => { })
        await c.env.DB.prepare('DELETE FROM team_members WHERE lower(trim(email)) = ?').bind(rejectedEmail).run().catch(() => { })
        await c.env.DB.prepare('DELETE FROM system_admins WHERE lower(trim(email)) = ?').bind(rejectedEmail).run().catch(() => { })
        await c.env.DB.prepare('DELETE FROM users WHERE lower(trim(email)) = ?').bind(rejectedEmail).run().catch(() => { })
    }

    return c.json({ ok: true })
})

app.post('/api/admin/members/role', async (c) => {
    const adminCheck = await requireSystemAdminSession(c)
    if (!adminCheck.ok) return adminCheck.response

    const body = await c.req.json().catch(() => ({})) as { line_user_id?: string; role?: string }
    const lineUserId = String(body.line_user_id || '').trim()
    const role = String(body.role || '').trim().toLowerCase()
    if (!lineUserId) return c.json({ error: 'line_user_id required' }, 400)
    if (!['admin', 'member', 'team'].includes(role)) return c.json({ error: 'invalid_role' }, 400)

    try {
        const result = await assignLineUserRole(c.env.DB, lineUserId, role, adminCheck.namespaceId)
        return c.json({ ok: true, ...result })
    } catch (error) {
...
async function handleLineEvent(event: any, env: Env, channelAccessToken: string) {
    ...
    if (event.type === 'postback') {
        await handleLinePostbackEvent({
            ...
            replyToken,
            lineUserId,
            data: String(event.postback?.data || '').trim(),
        })
        return
    }
...
async function handleLineTextMessage(params: {
    ...
    if (waitingState?.awaitingStep === 'cover') {
        if (!waitingState.coverPickerOpened && lowerText === 'เลือกปก') {
            try {
                ...
                console.error('[LINE] Cover option prompt error:', e instanceof Error ? e.message : String(e))
                await lineReply(replyToken, channelAccessToken, [
        ...
        if (isLineCoverReshuffleCommand(text)) {
            ...
            try {
                ...
                console.error('[LINE] Cover reshuffle error:', e instanceof Error ? e.message : String(e))
                await lineReply(replyToken, channelAccessToken, [
                    { type: 'text', text: 'เกิดข้อผิดพลาดในการสุ่มหน้าปกใหม่ กรุณาลองอีกครั้ง' },
                ]).catch(() => { })
            }
            return
        }
        ...
        await lineReply(replyToken, channelAccessToken, [
            buildLineCoverPickerFlex({
                ...
                namespaceId,
                workerUrl: String(env.WORKER_URL || '').trim(),
                coverOptions: waitingState.coverOptions && waitingState.coverOptions.length > 0
                    ? waitingState.coverOptions
                    : buildLineCoverOptions(waitingState.id, 10),
            }),
        ])
    ...
    if (waitingState?.awaitingStep === 'caption') {
        ...
        if (isLineCaptionEditCommand(text)) {
            await lineReply(replyToken, channelAccessToken, [
                { type: 'text', text: 'พิมพ์แคปชั่นมาได้เลย' },
            ])
            return
        }
        ...
        if (lowerText === 'วิธีใช้งาน' || lowerText === 'help' || lowerText === 'ส่งวิดีโอ') {
            await lineReply(replyToken, channelAccessToken, [
                buildLineCaptionPromptFlex({
                    title: 'รอแคปชั่น',
                    message: 'ส่งแคปชั่นมาเลย หรือกดข้ามให้ AI คิดให้',
                    videoId: waitingState.id,
                    namespaceId,
                    manualCaption: waitingState.manualCaption,
                }),
        ...
        const captionCandidate = normalizeManualCaption(stripLineAffiliateLinksFromText(text))
        if (!captionCandidate || /^https?:\/\/\S+$/i.test(text)) {
            await lineReply(replyToken, channelAccessToken, [
                buildLineCaptionPromptFlex({
                    title: 'รอแคปชั่น',
                    message: 'ส่งแคปชั่นมาเลย หรือกดข้ามให้ AI คิดให้',
    ...
    if (!shopeeLink && !lazadaLink) {
        ...
        if (waitingState) {
            ...
            await lineReply(replyToken, channelAccessToken, [
                buildLineAffiliatePromptFlex({
                    title: hasShopee ? 'รอ Lazada' : 'รอ Shopee',
    ...
    if (!updatedLazada) {
        ...
        await lineReply(replyToken, channelAccessToken, [
            buildLineAffiliatePromptFlex({
                ...
                brand: 'lazada',
                message: 'ส่งลิงก์ Lazada มาเลย',
            }),
        ])
        return
    }

    // Both links are present — ask for caption before processing
    try {
        await promptLineCaptionCollection({
            env,
            bucket,
            ...
            waitingState: {
                ...waitingState,
                sourceType: waitingState.sourceType || 'line_video',
                sourceLabel: waitingState.sourceLabel || 'LINE video',
                shopeeLink: updatedShopee,
                lazadaLink: updatedLazada,
                awaitingStep: 'caption',
...
async function handleLinePostbackEvent(params: {
    env: Env
    bucket: R2Bucket
    namespaceId: string
    channelAccessToken: string
...
app.post('/api/telegram/:token?', async (c) => {
    ...
    const sendAdminPanelLink = async () => {
        ...
        await sendTelegram(token, 'sendMessage', {
            chat_id: chatId,
            text: '✅ เปิด Admin Panel ได้เลย',
            reply_markup: { inline_keyboard: [[{ text: '⚙️ เปิด Admin Panel', web_app: { url: adminUrl } }]] },
        })
    }

    const tryHandleAdminPasswordInput = async (_inputText: string) => {
        return false
    }
        ...
        if (cb) {
            ...
            if (action.startsWith('del_channel:')) {
                ...
                if (ch) {
                    ...
                    const webAppUrl = getAppWebBaseUrl(c.env)
...
แต่ละช่องมีแดชบอร์ดและเพจแยกกัน`,
    ...
    const hasScopedTelegramSession = currentBotId
        ? await hasTelegramWorkspaceAccess(c.env.DB, String(chatId), currentBotId).catch(() => false)
        : false
    const ownerAccess = currentBotId && !hasScopedTelegramSession
        ? await claimOrValidateChannelOwner(
            c.env.DB,
            currentBotId,
            String(chatId),
            String(msg?.from?.first_name || cb?.from?.first_name || 'owner'),
        ).catch(() => null)
        : null
    if (currentBotId && !hasScopedTelegramSession && ownerAccess && !ownerAccess.allowed) {
        await sendTelegram(token, 'sendMessage', {
            chat_id: chatId,
            text: '🚫 กรุณาเปิด Workspace แล้ว login ด้วยอีเมลก่อนใช้งานบอท',
        }).catch(() => null)
        return c.text('ok')
    }
    ...
    if (text === '/logout') {
        ...
        const appUrl = getAppWebBaseUrl(c.env)
        await sendTelegram(token, 'sendMessage', {
            chat_id: chatId,
            text: '✅ ออกจากระบบแล้ว\n\nกด "เปิด Workspace" เพื่อเข้าใช้งานอีกครั้ง',
    ...
    // /start and /menu should always surface the Mini App entrypoint.
    if (text === '/start' || text === '/menu') {
        const appUrl = getAppWebBaseUrl(c.env)
        const hasSession = Boolean(userRecord?.session_token)
        await sendTelegram(token, 'sendMessage', {
            chat_id: chatId,
            text: hasSession
                ? '🚀 กด "เปิด Workspace" เพื่อเข้าใช้งาน'
                : '👋 กรุณาเปิด Workspace เพื่อเริ่มใช้งาน',
            reply_markup: {
                inline_keyboard: [[
    ...
    try {
        ...
        const normalizeWaitingVideoState = (input: Partial<WaitingVideoState> | null | undefined): WaitingVideoState | null => {
            const id = String(input?.id || '').trim()
            const videoUrl = String(input?.videoUrl || '').trim()
            if (!id || !videoUrl) return null
            const shopeeLink = extractShopeeLink(String(input?.shopeeLink || ''))
            const lazadaLink = extractLazadaLink(String(input?.lazadaLink || ''))
            const sourceType: InboxVideoSourceType = String(input?.sourceType || '').trim().toLowerCase() === 'xhs_url'
                ? 'xhs_url'
                : 'telegram_video'
            const sourceLabel = sourceType === 'telegram_video'
                ? 'Telegram video'
                : String(input?.sourceLabel || '').trim().slice(0, 500)
            return {
                id,
                videoUrl,
                sourceType,
        ...
        const getIntakeDuplicateKey = (input: Partial<WaitingVideoState> | Partial<InboxVideoRecord> | null | undefined): string => {
            const sourceType: InboxVideoSourceType = String(input?.sourceType || '').trim().toLowerCase() === 'xhs_url'
                ? 'xhs_url'
                : 'telegram_video'
            return getNamespaceVideoDuplicateKey({
                namespace_id: String(c.get('botId') || '').trim(),
                sourceType,
                sourceLabel: String(input?.sourceLabel || '').trim(),
                videoUrl: String(input?.videoUrl || '').trim(),
            })
        }
        const getWaitingVideoState = async (): Promise<WaitingVideoState | null> => {
            const waitingObj = await c.get('bucket').get(waitingVideoKey)
            if (!waitingObj) return null
            const waitingState = normalizeWaitingVideoState(await waitingObj.json() as Partial<WaitingVideoState>)
            if (!waitingState) {
        ...
        const saveWaitingVideoState = async (state: WaitingVideoState) => {
            await c.get('bucket').put(waitingVideoKey, JSON.stringify(state), {
                httpMetadata: { contentType: 'application/json' },
            })
        }
        ...
        const findExistingDuplicateInboxVideo = async (state: WaitingVideoState): Promise<
            ...
            | null
        > => {
            const duplicateKey = getIntakeDuplicateKey(state)
            if (!duplicateKey) return null

            const waitingState = await getWaitingVideoState()
            if (waitingState && getIntakeDuplicateKey(waitingState) === duplicateKey) {
                return { kind: 'waiting', item: waitingState }
            }

            const inboxRecords = await listInboxVideoRecords(c.get('bucket'))
            const existing = inboxRecords.find((record) => getIntakeDuplicateKey(record) === duplicateKey)
            return existing ? { kind: 'inbox', item: existing } : null
        }
        const getMissingWaitingLink = (state: WaitingVideoState): 'shopee' | 'lazada' | null => {
            if (!String(state.shopeeLink || '').trim()) return 'shopee'
            if (!String(state.lazadaLink || '').trim()) return 'lazada'
            return null
        }
        const promptWaitingLink = async (state: WaitingVideoState) => {
            const missing = getMissingWaitingLink(state)
            if (missing === 'lazada') {
                await sendTelegram(token, 'sendMessage', {
                    chat_id: chatId,
        ...
        const upsertInboxFromWaitingState = async (state: WaitingVideoState) => {
            const existing = await getInboxVideoRecord(c.get('bucket'), state.id)
            return putInboxVideoRecord(c.get('bucket'), {
                ...existing,
                id: state.id,
                videoUrl: state.videoUrl,
                chatId: Number(chatId),
                createdAt: String(existing?.createdAt || '').trim() || new Date().toISOString(),
                updatedAt: new Date().toISOString(),
                sourceType: state.sourceType,
                sourceLabel: state.sourceLabel,
                shopeeLink: state.shopeeLink,
                lazadaLink: state.lazadaLink,
                linkRecordedAt: existing?.linkRecordedAt,
            })
        }
        const sendReadyInboxMessage = async (result: InboxProcessingStartResult) => {
            const targetId = String(result.item.id || '').trim()
            const processingUrl = buildScopedTabWebAppUrl(
                getAppWebBaseUrl(c.env),
                String(c.get('botId') || ''),
                'processing',
            )
            const galleryUrl = buildScopedTabWebAppUrl(
                getAppWebBaseUrl(c.env),
                String(c.get('botId') || ''),
                'gallery',
                { q: targetId },
            ...
            if (result.outcome === 'already_processed') {
                await sendTelegram(token, 'sendMessage', {
                    chat_id: chatId,
                    text: '♻️ คลิปนี้เคยประมวลผลแล้ว\nผมอัปเดตลิงก์ให้รายการเดิมเรียบร้อย',
                    reply_markup: {
                        inline_keyboard: [[{ text: '🎞 เปิด Gallery', web_app: { url: galleryUrl } }]],
                    },
                })
                return
            }

            const status = result.outcome === 'started' ? result.job.status : result.status
            const text = result.outcome === 'already_running'
                ? (status === 'queued'
                    ? '♻️ คลิปนี้อยู่ในคิวประมวลผลแล้ว'
                    : '♻️ คลิปนี้กำลังประมวลผลอยู่แล้ว')
                : (status === 'queued'
                    ? '✅ รับลิงก์ครบแล้ว\nผมส่งคลิปเข้า Queue ให้แล้ว ระบบจะประมวลผลอัตโนมัติ'
                    : '✅ รับลิงก์ครบแล้ว\nผมเริ่มประมวลผลให้อัตโนมัติแล้ว')
            await sendTelegram(token, 'sendMessage', {
                chat_id: chatId,
                text,
                reply_markup: {
                    inline_keyboard: [[{ text: '⚙️ ดู Processing', web_app: { url: processingUrl } }]],
                },
            })
        }
        ...
        const handleVideoInput = async (
            ...
            const waitingState = normalizeWaitingVideoState({
                ...
                lazadaLink: initialLinks?.lazadaLink,
            })
            if (!waitingState) {
            ...
            const duplicate = await findExistingDuplicateInboxVideo(waitingState)
            if (duplicate?.kind === 'waiting') {
                const mergedWaitingState = normalizeWaitingVideoState({
                    ...duplicate.item,
                    shopeeLink: initialLinks?.shopeeLink || duplicate.item.shopeeLink,
                    lazadaLink: initialLinks?.lazadaLink || duplicate.item.lazadaLink,
                })
                if (mergedWaitingState) {
                    if (getMissingWaitingLink(mergedWaitingState)) {
                        await saveWaitingVideoState(mergedWaitingState)
                        await sendTelegram(token, 'sendMessage', {
                            chat_id: chatId,
                            text: '♻️ คลิปนี้มีอยู่แล้วในรายการรอลิงก์ ผมอัปเดตข้อมูลให้รายการเดิมแทน',
                        })
                        await promptWaitingLink(mergedWaitingState)
                    } else {
            ...
            if (duplicate?.kind === 'inbox') {
                const mergedRecord = await putInboxVideoRecord(c.get('bucket'), {
                    ...duplicate.item,
                    shopeeLink: initialLinks?.shopeeLink || duplicate.item.shopeeLink,
                    lazadaLink: initialLinks?.lazadaLink || duplicate.item.lazadaLink,
                    updatedAt: new Date().toISOString(),
                ...
                if (mergedRecord.status === 'ready') {
                    ...
                    await sendTelegram(token, 'sendMessage', {
                        chat_id: chatId,
                        text: '♻️ คลิปนี้มีอยู่แล้วในคลังต้นฉบับ ผมอัปเดตลิงก์ให้รายการเดิมแทน',
                    })
                }
                await c.get('bucket').put(dedupKey, 'processing')
                return
        ...
        if (shopeeLink || lazadaLink) {
            const waitingState = await getWaitingVideoState()
            if (waitingState) {
                const nextState = normalizeWaitingVideoState({
                    ...waitingState,
                    shopeeLink: shopeeLink || waitingState.shopeeLink,
                    lazadaLink: lazadaLink || waitingState.lazadaLink,
                })
                if (!nextState) {
                    await c.get('bucket').put(dedupKey, 'processing')
                    return c.text('ok')
                ...
                await saveWaitingVideoState(nextState)
                await promptWaitingLink(nextState)
                await c.get('bucket').put(dedupKey, 'processing')
                return c.text('ok')
            }
            await sendTelegram(token, 'sendMessage', {
...
type InboxVideoRecord = {
    ...
    linkRecordedAt?: string
    manualCaption?: string
    captionProvidedAt?: string
}

function normalizeInboxVideoRecord(input: Partial<InboxVideoRecord> | null | undefined): InboxVideoRecord | null {
    const id = String(input?.id || '').trim()
    const videoUrl = String(input?.videoUrl || '').trim()
    const chatId = Number(input?.chatId || 0)
    if (!id || !videoUrl || !Number.isFinite(chatId) || chatId < 0) return null

    const shopeeLink = pickFirstShopeeUrl(String(input?.shopeeLink || '')) || ''
    const lazadaLink = pickFirstLazadaUrl(String(input?.lazadaLink || '')) || ''
    const shopeeOriginalLink = pickFirstShopeeUrl(String(input?.shopeeOriginalLink || '')) || shopeeLink
    const lazadaOriginalLink = pickFirstLazadaUrl(String(input?.lazadaOriginalLink || '')) || lazadaLink
    const manualCaption = normalizeManualCaption(input?.manualCaption)
    ...
    return {
        ...
        ...(String(input?.linkRecordedAt || '').trim() ? { linkRecordedAt: String(input?.linkRecordedAt || '').trim() } : {}),
        ...(manualCaption ? { manualCaption } : {}),
        ...(String(input?.captionProvidedAt || '').trim() ? { captionProvidedAt: String(input?.captionProvidedAt || '').trim() } : {}),
    }
}

async function getInboxVideoRecord(bucket: R2Bucket, id: string): Promise<InboxVideoRecord | null> {
    const normalizedId = String(id || '').trim()
    if (!normalizedId) return null
    const obj = await bucket.get(`_inbox/${normalizedId}.json`)
    if (!obj) return null
    const raw = await obj.json().catch(() => null) as Partial<InboxVideoRecord> | null
    const record = normalizeInboxVideoRecord(raw)
    if (!record) {
        await bucket.delete(`_inbox/${normalizedId}.json`).catch(() => { })
        return null
    }
    return record
}

async function putInboxVideoRecord(bucket: R2Bucket, input: Partial<InboxVideoRecord>): Promise<InboxVideoRecord> {
    const record = normalizeInboxVideoRecord(input)
    if (!record) throw new Error('invalid_inbox_video_record')
    await bucket.put(`_inbox/${record.id}.json`, JSON.stringify(record), {
        httpMetadata: { contentType: 'application/json' },
    })
    return record
}
...
async function getInboxProcessingStateMap(bucket: R2Bucket): Promise<Map<string, InboxProcessingState>> {
    const stateMap = new Map<string, InboxProcessingState>()

    const applyState = async (prefix: '_queue/' | '_processing/', fallbackState: InboxProcessingState) => {
        const list = await bucket.list({ prefix })
        for (const obj of list.objects) {
            const id = obj.key.replace(prefix, '').replace(/\.json$/i, '').trim()
            if (!id) continue

            let nextState = fallbackState
            try {
                const payload = await bucket.get(obj.key)
                const json = payload ? await payload.json().catch(() => ({} as Record<string, unknown>)) as Record<string, unknown> : {}
                const rawStatus = String(json.status || '').trim().toLowerCase()
                if (rawStatus === 'failed') continue
                if (rawStatus === 'queued') nextState = 'queued'
                else if (rawStatus === 'processing') nextState = 'processing'
            } catch {
                // fall back to the prefix-derived state
            }
    ...
    await applyState('_queue/', 'queued')
    await applyState('_processing/', 'processing')
    return stateMap
}
...
function toInboxVideoResponse(
    ...
    return {
        ...
        shopeeLink: String(record.shopeeLink || '').trim(),
        lazadaLink: String(record.lazadaLink || '').trim(),
        manualCaption: normalizeManualCaption(record.manualCaption),
        hasManualCaption: !!normalizeManualCaption(record.manualCaption),
...
function pickPreferredInboxArchiveVideo(current: Record<string, unknown>, next: Record<string, unknown>): Record<string, unknown> {
    const currentCanDelete = current.canDelete === true ? 1 : 0
    const nextCanDelete = next.canDelete === true ? 1 : 0
    if (currentCanDelete !== nextCanDelete) {
    ...
    const nextTs = new Date(String(next.updatedAt || next.createdAt || '')).getTime()
    if (currentTs !== nextTs) {
        return nextTs >= currentTs ? next : current
    }

    return String(next.id || '').localeCompare(String(current.id || '')) <= 0 ? next : current
}

function getInboxArchiveDuplicateKey(video: Record<string, unknown>): string {
    const originalUrl = normalizeVideoDuplicateValue(video.originalUrl || video.original_url)
    if (originalUrl) return `original::${originalUrl}`
    ...
    if (sourceType === 'xhs_url' && sourceLabel) return `xhs::${sourceLabel}`
    if (videoUrl) return `video::${videoUrl}`
    if (previewUrl) return `preview::${previewUrl}`
    return `id::${String(video.id || '').trim()}`
}

function dedupeInboxArchiveVideos(videos: Array<Record<string, unknown>>): Array<Record<string, unknown>> {
    const byKey = new Map<string, Record<string, unknown>>()

    for (const video of videos) {
        const dedupeKey = getInboxArchiveDuplicateKey(video)
        const previous = byKey.get(dedupeKey)
        if (!previous) {
            byKey.set(dedupeKey, video)
            continue
        }
        byKey.set(dedupeKey, pickPreferredInboxArchiveVideo(previous, video))
    }
...
async function listNamespaceOriginalArchiveVideos(params: {
    ...
    const assetEntries = Array.from(originalAssetMap.entries())
    const metaEntries = await Promise.all(assetEntries.map(async ([id, asset]) => {
        const metaObj = await params.bucket.get(`videos/${id}.json`).catch(() => null)
        const meta = metaObj ? await metaObj.json().catch(() => ({} as Record<string, unknown>)) as Record<string, unknown> : {}
        return { id, asset, meta }
    }))
    const metaById = new Map(metaEntries.map((entry) => [entry.id, entry.meta]))
    ...
    for (const { id, asset, meta } of metaEntries) {
        ...
        const row = stateByVideoId.get(id)
        const hasMeaningfulMeta = !!(
            String(meta.createdAt || meta.updatedAt || meta.title || meta.originalUrl || meta.original_url || meta.publicUrl || meta.thumbnailUrl || meta.originalThumbnailUrl || meta.original_thumbnail_url || meta.script || '').trim()
            || normalizeMetaShopeeLink(meta)
            || normalizeMetaLazadaLink(meta)
            || normalizeManualCaption(meta.manualCaption || meta.caption || '')
        )
        if (!row && !asset.hasProcessedVideo && !hasMeaningfulMeta) continue

        const shopeeLink = String(row?.shopee_link || normalizeMetaShopeeLink(meta) || '').trim()
        const lazadaLink = String(row?.lazada_link || normalizeMetaLazadaLink(meta) || '').trim()
        const createdAt = String(meta.createdAt || asset.originalUploadedAt || asset.processedUploadedAt || new Date().toISOString()).trim()
        const updatedAt = String(meta.updatedAt || asset.processedUploadedAt || asset.originalUploadedAt || createdAt).trim()
        const title = String(meta.title || '').trim()
        const sourceLabel = title || 'Original video'
        ...
        merged.set(id, {
            ...
            sourceType: 'telegram_video',
            sourceLabel,
            manualCaption: normalizeManualCaption(meta.manualCaption || meta.caption || ''),
            hasManualCaption: !!normalizeManualCaption(meta.manualCaption || meta.caption || ''),
            shopeeLink,
            lazadaLink,
    ...
    return dedupeInboxArchiveVideos(Array.from(merged.values())).map((video) => {
...
async function recordInboxLinkSubmissionIfNeeded(params: {
...
}): Promise<InboxVideoRecord> {
    if (
        params.item.status !== 'ready'
        || String(params.item.linkRecordedAt || '').trim()
        || Number(params.item.chatId || 0) <= 0
    ) return params.item
    ...
    await recordLinkSubmission({
        db: params.db,
        namespaceId: params.namespaceId,
        telegramId: String(params.item.chatId),
        videoId: params.item.id,
        shopeeLink: String(params.item.shopeeLink || '').trim(),
    }).then(() => {
        recorded = true
    }).catch((e) => {
        console.error(`[DASHBOARD] failed to record inbox link submission: ${e instanceof Error ? e.message : String(e)}`)
    })
    if (!recorded) return params.item

    const nextRecord = await putInboxVideoRecord(params.bucket, {
        ...params.item,
        linkRecordedAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
    })
    return nextRecord
}

async function copyBucketObjectIfMissing(params: {
    sourceBucket: R2Bucket
    targetBucket: R2Bucket
    sourceKey: string
    targetKey: string
    fallbackContentType?: string
    replaceIfSizeDiffers?: boolean
}): Promise<boolean> {
    const sourceKey = String(params.sourceKey || '').trim()
    const targetKey = String(params.targetKey || '').trim()
    if (!sourceKey || !targetKey) return false

    const sourceHead = await params.sourceBucket.head(sourceKey).catch(() => null)
    if (!sourceHead) return false

    const existing = await params.targetBucket.head(targetKey).catch(() => null)
    const shouldReplaceExisting = !!existing && (
        isLikelyInvalidStoredVideoObject(existing)
        || (
            params.replaceIfSizeDiffers === true
            && Number(existing.size || 0) !== Number(sourceHead.size || 0)
        )
    )
    if (existing && !shouldReplaceExisting) return true

    const obj = await params.sourceBucket.get(sourceKey).catch(() => null)
    ...
    await params.targetBucket.put(targetKey, obj.body, contentType
        ? { httpMetadata: { contentType } }
        : undefined
    )
    return true
...
async function resolvePrimaryAdminNamespaceId(db: D1Database): Promise<string> {
    const row = await db.prepare(
        `SELECT sa.email, en.namespace_id
         FROM system_admins sa
         JOIN email_namespaces en ON en.email = sa.email
         LIMIT 1`
    ).first() as { namespace_id?: string } | null
...
async function syncImportedOriginalIntoNamespace(params: {
    ...
    const targetNamespaceId = String(params.targetNamespaceId || '').trim()
    const sourceNamespaceId = String(params.sourceNamespaceId || '').trim()
    const videoId = String(params.sourceVideo.id || '').trim()
    if (!targetNamespaceId || !sourceNamespaceId || !videoId || targetNamespaceId === sourceNamespaceId) return null

    const sourceBucket = params.sourceBucket || (new BotBucket(params.env.BUCKET, sourceNamespaceId) as unknown as R2Bucket)
    const sourceOriginalKey = await resolveGalleryAssetKey(sourceBucket, videoId, 'original').catch(() => '')
    if (!sourceOriginalKey) return null
    const sourceOriginalHead = await sourceBucket.head(sourceOriginalKey).catch(() => null)
    if (!sourceOriginalHead) return null

    const existingMetaObj = await params.targetBucket.get(`videos/${videoId}.json`).catch(() => null)
    if (existingMetaObj) {
        const existingMeta = await existingMetaObj.json().catch(() => null) as Record<string, unknown> | null
        const targetPublicHead = await params.targetBucket.head(`videos/${videoId}.mp4`).catch(() => null)
        if (
            existingMeta
            && isImportedVideoMetaPointingAtSourceNamespace({
                meta: existingMeta,
                targetNamespaceId,
                sourceNamespaceId,
            })
            ...
            await clearNamespaceProcessedArtifacts(params.targetBucket, videoId)
        }
    }

    const targetOriginalKey = getPrimaryOriginalVideoAssetKey(videoId)
    await copyBucketObjectIfMissing({
        sourceBucket,
        targetBucket: params.targetBucket,
        sourceKey: sourceOriginalKey,
        targetKey: targetOriginalKey,
        fallbackContentType: sourceOriginalHead.httpMetadata?.contentType || 'video/mp4',
        replaceIfSizeDiffers: true,
    })

    const existing = await getInboxVideoRecord(params.targetBucket, videoId).catch(() => null)
    const sourceTypeRaw = String(params.sourceVideo.sourceType || params.sourceVideo.source_type || '').trim().toLowerCase()
    const sourceType: InboxVideoSourceType = sourceTypeRaw === 'xhs_url'
        ? 'xhs_url'
        : sourceTypeRaw === 'telegram_video'
            ? 'telegram_video'
            : 'line_video'
    const sourceLabelBase = String(
        params.sourceVideo.sourceLabel ||
        params.sourceVideo.source_label ||
        params.sourceVideo.title ||
        ''
    ).trim()
    const importedLabel = (sourceLabelBase
        ? `${sourceLabelBase} • imported from ${sourceNamespaceId}`
        : `imported from ${sourceNamespaceId}`
    ).slice(0, 500)
    const shopeeLink = getVideoSourceShopeeLink(params.sourceVideo) || ''
    const lazadaLink = getVideoSourceLazadaLink(params.sourceVideo) || ''
    const sourceManualCaption = normalizeManualCaption(params.sourceVideo.manualCaption || params.sourceVideo.caption || '')
    ...
    const nextRecord = await putInboxVideoRecord(params.targetBucket, {
        id: videoId,
        videoUrl: getVideoOriginalUrlForNamespace(params.env.R2_PUBLIC_URL, targetNamespaceId, videoId),
        ...
        shopeeLink,
        lazadaLink,
        shopeeOriginalLink: shopeeLink,
        lazadaOriginalLink: lazadaLink,
        linkRecordedAt: String(existing?.linkRecordedAt || createdAt).trim() || createdAt,
        manualCaption: manualCaption || existing?.manualCaption || '',
        captionProvidedAt: manualCaption
    ...
    ).bind(targetNamespaceId, videoId, sourceNamespaceId, updatedAt).run().catch(() => { })

    await backfillOriginalThumbnail(params.env, targetNamespaceId, videoId).catch(() => ({ generated: false, thumbnailUrl: '' }))

    return nextRecord
}

async function syncWaitingVideoIntoPrimaryAdminNamespace(params: {
    env: Env
    sourceNamespaceId: string
    sourceBucket: R2Bucket
    waitingState: LineWaitingVideoState
    ...
    const waitingState = normalizeLineWaitingVideoState(params.waitingState)
    if (!waitingState || !waitingState.shopeeLink || !waitingState.lazadaLink) return null

    const adminNamespaceId = await resolvePrimaryAdminNamespaceId(params.env.DB).catch(() => '')
    const sourceNamespaceId = String(params.sourceNamespaceId || '').trim()
    if (!adminNamespaceId || !sourceNamespaceId || adminNamespaceId === sourceNamespaceId) return null

    const adminBucket = new BotBucket(params.env.BUCKET, adminNamespaceId) as unknown as R2Bucket
    return syncImportedOriginalIntoNamespace({
        env: params.env,
        targetNamespaceId: adminNamespaceId,
        targetBucket: adminBucket,
        sourceNamespaceId,
...
async function prepareInboxVideoForManagedLinks(params: {
    ...
    const nextConvertedAt = new Date().toISOString()
    const shopeeTrace: {
        utmSource?: string | null
        status?: 'disabled' | 'shortened' | 'fallback'
        error?: string | null
    } = {}
    let nextShopeeLink = await shortenShopeeLinkForNamespace({
        env: params.env,
        namespaceId,
    ...
    const shopeeReady = (
        ...
        && shopeeTrace.status === 'shortened'
        && (
            !normalizedExpectedUtmId
            || normalizeShortlinkExpectedUtmId(String(shopeeTrace.utmSource || '')) === normalizedExpectedUtmId
        )
    )
    if (!shopeeReady) {
    ...
    let nextLazadaLink = await shortenLazadaLinkForNamespace({
        env: params.env,
        namespaceId,
    ...
    await upsertNamespaceVideoState(params.env.DB, namespaceId, item.id, {
        shopee_link: nextShopeeLink,
        lazada_link: nextLazadaLink,
        shopee_original_link: sourceShopeeLink,
        lazada_original_link: sourceLazadaLink,
        shopee_converted_at: nextConvertedAt,
        lazada_converted_at: nextConvertedAt,
        lazada_member_id: normalizedMemberId,
    })

    if (nextShopeeLink === item.shopeeLink && nextLazadaLink === item.lazadaLink) {
        return item
    }

    return putInboxVideoRecord(params.bucket, {
        ...item,
        shopeeLink: nextShopeeLink,
        lazadaLink: nextLazadaLink,
        shopeeOriginalLink: sourceShopeeLink,
        lazadaOriginalLink: sourceLazadaLink,
        updatedAt: new Date().toISOString(),
    })
}

async function startInboxVideoProcessing(params: {
    env: Env
    bucket: R2Bucket
    ...
    const nowIso = new Date().toISOString()
    const item = normalizeInboxVideoRecord(params.item)
    if (!item || item.status !== 'ready') throw new Error('inbox_video_not_ready')

    await params.bucket.put(`_link_context/${item.id}.json`, JSON.stringify({
        id: item.id,
        videoUrl: item.videoUrl,
        shopeeLink: String(item.shopeeLink || '').trim(),
        lazadaLink: String(item.lazadaLink || '').trim(),
        shopeeOriginalLink: String(item.shopeeOriginalLink || item.shopeeLink || '').trim(),
        lazadaOriginalLink: String(item.lazadaOriginalLink || item.lazadaLink || '').trim(),
        manualCaption: normalizeManualCaption(item.manualCaption),
        captionProvidedAt: String(item.captionProvidedAt || '').trim(),
        chatId: item.chatId,
        createdAt: nowIso,
    }), {
        httpMetadata: { contentType: 'application/json' },
...
type InboxProcessingStartResult =
    ...
    | { outcome: 'already_processed'; item: InboxVideoRecord; id: string }

async function ensureInboxVideoProcessingStarted(params: {
    env: Env
    bucket: R2Bucket
    executionCtx: ExecutionContext
    botId: string
    ...
    const waitingObj = await params.bucket.get(waitingKey)
    if (waitingObj) {
        const waiting = await waitingObj.json().catch(() => ({} as Record<string, unknown>)) as Record<string, unknown>
        if (String(waiting.id || '').trim() === item.id) {
            await params.bucket.delete(waitingKey).catch(() => { })
        }
    }
...
app.post('/api/desktop-prepare', async (c) => {
    ...
    try {
        ...
        const sessionUser = await c.env.DB.prepare(
            'SELECT email, namespace_id FROM users WHERE session_token = ? LIMIT 1'
        ).bind(token).first().catch(() => null) as { email?: string; namespace_id?: string } | null

        const namespaceId = String(sessionUser?.namespace_id || c.get('botId') || '').trim()
        const email = String(sessionUser?.email || '').trim().toLowerCase()
        if (!namespaceId || !email) return c.json({ error: 'unauthorized' }, 401)

        const currentNamespaceId = String(c.get('botId') || '').trim()
        const bucket = currentNamespaceId === namespaceId
            ? c.get('bucket')
        ...
        const sourceMode = String(body.sourceMode || '').trim().toLowerCase() === 'upload' ? 'upload' : 'url'
        let videoId = String(body.videoId || '').trim()
        let originalVideoUrl = ''
        let thumbnailUrl = ''
        let sourceType: InboxVideoSourceType = 'telegram_video'
        let sourceLabel = 'Desktop upload'

        if (sourceMode === 'upload') {
            if (!videoId) return c.json({ error: 'video_id_required' }, 400)
            const originalKey = getPrimaryOriginalVideoAssetKey(videoId)
            const originalObj = await bucket.head(originalKey).catch(() => null)
            if (!originalObj) return c.json({ error: 'uploaded_video_not_found' }, 404)

            originalVideoUrl = getVideoOriginalUrlForNamespace(c.env.R2_PUBLIC_URL, namespaceId, videoId)
            sourceType = 'telegram_video'
            sourceLabel = 'Desktop upload'
            thumbnailUrl = await backfillOriginalThumbnail(c.env, namespaceId, videoId)
                .then((result) => String(result.thumbnailUrl || '').trim())
                .catch(() => '')
            if (!thumbnailUrl) {
                thumbnailUrl = getVideoOriginalThumbnailUrlForNamespace(c.env.R2_PUBLIC_URL, namespaceId, videoId)
            }
        } else {
            const sourceUrl = String(body.sourceUrl || '').trim()
            if (!sourceUrl) return c.json({ error: 'source_url_required' }, 400)
            if (!videoId) videoId = crypto.randomUUID().replace(/-/g, '').slice(0, 8)

            const isXhs = /https?:\/\/(xhslink\.com|www\.xiaohongshu\.com)\S+/i.test(sourceUrl)
            sourceType = isXhs ? 'xhs_url' : 'telegram_video'
            sourceLabel = isXhs
                ? sourceUrl
                : (String(body.sourceLabel || '').trim().slice(0, 500) || 'Desktop URL')
            ...
            originalVideoUrl = materialized.originalUrl
            thumbnailUrl = String(materialized.thumbnailUrl || '').trim()
                || getVideoOriginalThumbnailUrlForNamespace(c.env.R2_PUBLIC_URL, namespaceId, videoId)
        }

        const workerBase = String(c.env.WORKER_URL || '').trim().replace(/\/+$/, '')
        const frameUrlBase = workerBase
            ? `${workerBase}/api/gallery/${encodeURIComponent(videoId)}/frame?namespace_id=${encodeURIComponent(namespaceId)}`
            : ''
...
app.post('/api/desktop-submit', async (c) => {
    const authCheck = await requireAuthSession(c)
    if (!authCheck.ok) return authCheck.response
    ...
    try {
        ...
        const body = await c.req.json().catch(() => ({})) as {
            sourceMode?: 'upload' | 'url' | 'prepared'
            videoId?: string
            sourceUrl?: string
            sourceLabel?: string
            sourceType?: InboxVideoSourceType
        ...
        const sourceModeRaw = String(body.sourceMode || '').trim().toLowerCase()
        const sourceMode = sourceModeRaw === 'upload'
            ? 'upload'
            : sourceModeRaw === 'prepared'
                ? 'prepared'
                : 'url'
        const shopeeLink = pickFirstShopeeUrl(String(body.shopeeLink || '')) || ''
        const lazadaLink = pickFirstLazadaUrl(String(body.lazadaLink || '')) || ''
        const manualCaption = normalizeManualCaption(body.manualCaption)
        if (!shopeeLink || !lazadaLink) {
            return c.json({ error: 'missing_affiliate_links' }, 400)
        }
        ...
        if (sourceMode === 'prepared') {
            if (!videoId) return c.json({ error: 'video_id_required' }, 400)
            const originalKey = getPrimaryOriginalVideoAssetKey(videoId)
            const originalObj = await bucket.head(originalKey).catch(() => null)
            if (!originalObj) return c.json({ error: 'prepared_video_not_found' }, 404)

            originalVideoUrl = getVideoOriginalUrlForNamespace(c.env.R2_PUBLIC_URL, namespaceId, videoId)
            const sourceTypeRaw = String(body.sourceType || '').trim().toLowerCase()
            sourceType = sourceTypeRaw === 'xhs_url'
                ? 'xhs_url'
                : sourceTypeRaw === 'line_video'
                    ? 'line_video'
                    : 'telegram_video'
            sourceLabel = String(body.sourceLabel || '').trim().slice(0, 500)
                || (sourceType === 'xhs_url' ? 'Desktop XHS URL' : 'Desktop upload')
            await backfillOriginalThumbnail(c.env, namespaceId, videoId).catch(() => ({ generated: false, thumbnailUrl: '' }))
        ...
        const chatId = await resolveSessionSubmissionChatId(c.env.DB, email)
        ...
        return c.json({
            ok: true,
            namespace_id: namespaceId,
            video: toInboxVideoResponse(inboxRecord, {
                namespaceId,
                originalUrl: originalVideoUrl,
                previewUrl: originalVideoUrl,
            }),
            job: result,
            processing_url: buildScopedTabWebAppUrl(getDesktopWebBaseUrl(c.env), namespaceId, 'processing'),
            gallery_url: buildScopedTabWebAppUrl(getDesktopWebBaseUrl(c.env), namespaceId, 'gallery'),
        })
    } catch (e) {
...
app.post('/api/inbox/:id/process', async (c) => {
    try {
        ...
        const currentNamespaceId = String(c.get('botId') || '').trim()
        let processBucket = c.get('bucket')

        // Try current namespace first
        let item = await getInboxVideoRecord(processBucket, id)

        // If not found and a different namespace was requested, look there and copy to current
        if (!item && requestedNamespaceId && requestedNamespaceId !== currentNamespaceId) {
            const sourceBucket = new BotBucket(c.env.BUCKET, requestedNamespaceId) as unknown as R2Bucket
            const sourceItem = await getInboxVideoRecord(sourceBucket, id)
            if (sourceItem) {
                item = await syncImportedOriginalIntoNamespace({
                    env: c.env,
                    targetNamespaceId: currentNamespaceId,
                    targetBucket: processBucket,
                    sourceNamespaceId: requestedNamespaceId,
                    sourceBucket,
                    sourceVideo: {
                        ...sourceItem,
                        id,
                ...
                if (!item) {
                    item = await putInboxVideoRecord(processBucket, {
                        ...sourceItem,
                        updatedAt: new Date().toISOString(),
                    })
                }
        ...
        // If still not found, check all namespaces (for system inbox videos)
        if (!item) {
            const sysAdmin = await isSystemAdmin(c.env.DB, (await c.env.DB.prepare('SELECT email FROM users WHERE session_token = ?').bind(getSessionTokenFromRequest(c)).first() as any)?.email || '').catch(() => false)
            if (sysAdmin) {
                const allNamespaces = await c.env.DB.prepare('SELECT DISTINCT namespace_id FROM line_users').all().catch(() => ({ results: [] }))
                for (const row of (allNamespaces.results || [])) {
                    const ns = String((row as any).namespace_id || '').trim()
                    if (!ns || ns === currentNamespaceId) continue
                    const srcBucket = new BotBucket(c.env.BUCKET, ns) as unknown as R2Bucket
                    const srcItem = await getInboxVideoRecord(srcBucket, id).catch(() => null)
                    if (srcItem) {
                        item = await syncImportedOriginalIntoNamespace({
                            env: c.env,
                            targetNamespaceId: currentNamespaceId,
                            targetBucket: processBucket,
                            sourceNamespaceId: ns,
        ...
        if (result.outcome === 'already_processed') {
            return c.json({ ok: true, job: { status: 'processed', id } })
        }
...
app.delete('/api/inbox/:id', async (c) => {
    try {
        ...
        if (targetNamespaceId !== currentNamespaceId) {
            return c.json({ error: 'cross_namespace_inbox_delete_forbidden' }, 403)
        }
...
app.post('/api/processing/:id/reprocess', async (c) => {
    try {
        ...
        const linkContext = linkContextObj
            ? await linkContextObj.json().catch(() => ({} as Record<string, unknown>)) as {
                videoUrl?: string
                shopeeLink?: string
                lazadaLink?: string
                manualCaption?: string
        ...
        const videoUrl = String(job.videoUrl || linkContext?.videoUrl || '').trim()
        const chatIdRaw = Number(job.chatId ?? linkContext?.chatId ?? 0)
        const chatId = Number.isFinite(chatIdRaw) && chatIdRaw >= 0 ? chatIdRaw : 0
        if (!videoUrl) {
        ...
        const queuedJob = {
            id,
            videoUrl,
            shopeeLink: String(job.shopeeLink || linkContext?.shopeeLink || '').trim(),
            lazadaLink: String(job.lazadaLink || linkContext?.lazadaLink || '').trim(),
            manualCaption: normalizeManualCaption(job.manualCaption || linkContext?.manualCaption || ''),
            chatId,
    ...
    try {
        ...
        if (videoId && linkContext) {
            ...
            if (metaObj) {
                ...
                const shopeeOriginalLink = pickFirstShopeeUrl(String(linkContext.shopeeOriginalLink || shopeeLink || '')) || ''
                const lazadaOriginalLink = pickFirstLazadaUrl(String(linkContext.lazadaOriginalLink || lazadaLink || '')) || ''
                const manualCaption = normalizeManualCaption(linkContext.manualCaption)
                let changed = false
                const nextStateFields: Partial<NamespaceVideoStateRow> = {}
                if (shopeeLink && !normalizeMetaShopeeLink(meta)) {
                    meta.shopeeLink = shopeeLink
                    meta.shopeeOriginalLink = getVideoSourceShopeeLink(meta) || shopeeOriginalLink || shopeeLink
                    if (!String(meta.linkSubmittedAt || '').trim()) {
                        meta.linkSubmittedAt = new Date().toISOString()
                    }
                    nextStateFields.shopee_link = shopeeLink
                    nextStateFields.shopee_original_link = String(meta.shopeeOriginalLink || '').trim()
                    changed = true
                }
                if (lazadaLink && !normalizeMetaLazadaLink(meta)) {
                    meta.lazadaLink = lazadaLink
                    meta.lazadaOriginalLink = getVideoSourceLazadaLink(meta) || lazadaOriginalLink || lazadaLink
                    nextStateFields.lazada_link = lazadaLink
                    nextStateFields.lazada_original_link = String(meta.lazadaOriginalLink || '').trim()
                    changed = true
                }
                if (manualCaption && normalizeManualCaption(meta.manualCaption || '') !== manualCaption) {
                    meta.manualCaption = manualCaption
                    meta.captionSource = 'manual'
                    meta.captionProvidedAt = String(linkContext.captionProvidedAt || '').trim() || new Date().toISOString()
                    changed = true
                }
                if (changed) {
        ...
        if (videoId) {
            await syncOriginalThumbnailToProcessedThumbnail({
                env: c.env,
                bucket: c.get('bucket'),
                namespaceId,
                videoId,
        ...
        if (linkContext && Number(linkContext.chatId || 0) > 0) {
            ...
            try {
                const ch = await c.env.DB.prepare('SELECT bot_token FROM channels WHERE bot_id = ?').bind(namespaceId).first() as { bot_token?: string } | null
                if (String(ch?.bot_token || '').trim()) {
                    botToken = String(ch?.bot_token || '').trim()
                }
            } catch {
                // fall back to default token
            }
            const completionUrl = buildScopedGalleryWebAppUrl(
                getAppWebBaseUrl(c.env),
                namespaceId,
                'lazada',
                videoId,
            ...
            await sendTelegram(botToken, 'sendMessage', {
                chat_id: Number(linkContext.chatId || 0),
                text: `✅ ประมวลผลเสร็จแล้ว\nVideo ID: ${videoId}`,
                reply_markup: {
                    inline_keyboard: [[{ text: '🧡 เปิด Gallery Lazada', web_app: { url: completionUrl } }]],
                },
            }).catch(() => null)
        }
...
app.get('/api/gallery', async (c) => {
    ...
    try {
        ...
        const bucket = c.get('bucket')
        const pageVideos = await Promise.all(page.videos.map(async (video) => {
            const currentVideo = video as Record<string, unknown>
            const videoId = String(currentVideo.id || '').trim()
            const currentManualCaption = normalizeManualCaption(currentVideo.manualCaption || currentVideo.caption || '')
            if (!videoId || currentManualCaption) return video

            const metaObj = await bucket.get(`videos/${videoId}.json`).catch(() => null)
            if (!metaObj) return video
            const meta = await metaObj.json().catch(() => ({} as Record<string, unknown>)) as Record<string, unknown>
            const manualCaption = normalizeManualCaption(meta.manualCaption || meta.caption || '')
            const captionProvidedAt = String(meta.captionProvidedAt || meta.caption_provided_at || '').trim()
            if (!manualCaption && !captionProvidedAt) return video

            return {
                ...currentVideo,
                manualCaption,
                hasManualCaption: !!manualCaption,
                captionProvidedAt: captionProvidedAt || undefined,
            }
        }))
...
app.post('/api/gallery/import', async (c) => {
    try {
        ...
        // 5. Import as original-only into admin namespace; never reuse processed output from source namespace
        const adminBucket = new BotBucket(c.env.BUCKET, adminNamespaceId) as unknown as R2Bucket
        const importedRecord = await syncImportedOriginalIntoNamespace({
            env: c.env,
            targetNamespaceId: adminNamespaceId,
            targetBucket: adminBucket,
            sourceNamespaceId,
        ...
        if (!importedRecord) return c.json({ error: 'Import failed' }, 500)
...
const getAllOriginalGallery = async (
    ...
    try {
        ...
        const ownerCheck = await isNamespaceOwner(c.env.DB, me.email)
...
app.put('/api/gallery/:id', async (c) => {
    ...
    try {
        ...
        const lazadaExpectedMemberId = await resolveNamespaceLazadaExpectedMemberId(c.env.DB, targetNamespaceId).catch(() => '')
        let changed = false
        let touchedAffiliateFields = false
        const nextStateFields: Partial<NamespaceVideoStateRow> = {}
        if (body.shopeeLink !== undefined) {
            ...
            if (normalizedShopeeLink) {
                ...
                meta.linkSubmittedAt = new Date().toISOString()
                nextStateFields.shopee_link = normalizedShopeeLink
                nextStateFields.shopee_original_link = String(meta.shopeeOriginalLink || '').trim()
                if (isLikelyConvertedShopeeLink(normalizedShopeeLink, shopeeExpectedUtmId)) {
                    ...
                    nextStateFields.shopee_converted_at = String(meta.shopeeConvertedAt || '').trim()
                } else {
                ...
                delete meta.shopeeConvertedAt
                delete meta.shopeeConversionError
                nextStateFields.shopee_link = ''
                nextStateFields.shopee_original_link = ''
                nextStateFields.shopee_converted_at = ''
            }
            changed = true
            touchedAffiliateFields = true
        }
        if (body.lazadaLink !== undefined) {
            ...
            if (normalizedLazadaLink) {
                ...
                nextStateFields.lazada_link = normalizedLazadaLink
                nextStateFields.lazada_original_link = String(meta.lazadaOriginalLink || '').trim()
                if (isLikelyConvertedLazadaLink(normalizedLazadaLink)) {
                    const inferredMemberId = normalizeLazadaMemberId(
                        String(meta.lazadaMemberId || meta.lazada_member_id || lazadaExpectedMemberId || '')
                    )
                    if (inferredMemberId) {
                        meta.lazadaMemberId = inferredMemberId
                        nextStateFields.lazada_member_id = inferredMemberId
                    }
                    meta.lazadaConvertedAt = new Date().toISOString()
                    delete meta.lazadaConversionError
                    nextStateFields.lazada_converted_at = String(meta.lazadaConvertedAt || '').trim()
                } else {
                ...
                delete meta.lazadaConvertedAt
                delete meta.lazadaConversionError
                if (!lazadaExpectedMemberId) delete meta.lazadaMemberId
                nextStateFields.lazada_link = ''
                nextStateFields.lazada_original_link = ''
                nextStateFields.lazada_converted_at = ''
                nextStateFields.lazada_member_id = ''
            }
            changed = true
            touchedAffiliateFields = true
        }
        ...
        if (body.manualCaption !== undefined) {
            const manualCaption = normalizeManualCaption(body.manualCaption)
            if (manualCaption) {
                meta.manualCaption = manualCaption
                meta.captionProvidedAt = new Date().toISOString()
            } else {
                delete meta.manualCaption
                delete meta.captionProvidedAt
            }
            changed = true
        }
        if (changed) {
...
app.get('/api/gallery/:id/frame', async (c) => {
    ...
    const seekSeconds = timeParam !== '' && Number.isFinite(timeRaw) ? Math.max(0, timeRaw) : undefined
    const seed = String(c.req.query('seed') || '').trim()
    const targetWidthRaw = Number(c.req.query('w') || c.req.query('width') || '')
    const targetHeightRaw = Number(c.req.query('h') || c.req.query('height') || '')
    const targetWidth = Number.isFinite(targetWidthRaw) ? Math.max(120, Math.min(1440, Math.round(targetWidthRaw))) : undefined
    const targetHeight = Number.isFinite(targetHeightRaw) ? Math.max(160, Math.min(2560, Math.round(targetHeightRaw))) : undefined
    ...
    try {
        ...
        if (!videoUrl) return c.text('Not found', 404)

        const thumbBytes = await generateThumbnailViaContainer(
            c.env,
            videoUrl,
            seed || `${targetNamespaceId}:${id}:cover:${Number.isFinite(seekSeconds) ? Number(seekSeconds || 0).toFixed(2) : 'auto'}`,
            seekSeconds,
            requestedFormat,
            targetWidth,
            targetHeight,
...
function normalizeGeminiApiKeyList(rawKeys: unknown): string[] {
...
async function getLegacySystemGeminiApiKeys(db: D1Database): Promise<{ keys: string[]; updatedAt: string | null }> {
    await ensureNamespaceSettingsTable(db)
    const rows = await db.prepare(
        `SELECT ns.value, ns.updated_at
    ...
    ).bind(NS_SETTING_GEMINI_API_KEY).all() as {
        results?: Array<{ value?: string; updated_at?: string }>
    }

    const keys = normalizeGeminiApiKeyList((rows.results || []).map((row) => row?.value || ''))
    const updatedAt = String(rows.results?.[0]?.updated_at || '').trim() || null
    return { keys, updatedAt }
}

async function clearLegacySystemGeminiApiKeys(db: D1Database): Promise<void> {
    await ensureNamespaceSettingsTable(db)
    await db.prepare(
        `DELETE FROM namespace_settings
         WHERE key = ?
           AND namespace_id IN (
             SELECT DISTINCT u.namespace_id
             FROM users u
             INNER JOIN system_admins sa
               ON lower(trim(sa.email)) = lower(trim(u.email))
             WHERE trim(coalesce(u.namespace_id, '')) <> ''
           )`
    ).bind(NS_SETTING_GEMINI_API_KEY).run()
}

async function getSystemGeminiApiKeyEntries(db: D1Database): Promise<{ keys: string[]; updatedAt: string | null; source: 'system' | 'legacy' | 'none' }> {
    await ensureNamespaceSettingsTable(db)
    const row = await db.prepare(
        'SELECT value, updated_at FROM namespace_settings WHERE namespace_id = ? AND key = ?'
    ).bind(SYSTEM_GEMINI_NAMESPACE_ID, NS_SETTING_GEMINI_API_KEYS).first() as { value?: string; updated_at?: string } | null
    ...
    const keys = normalizeGeminiApiKeyList(parsed)
    if (keys.length > 0) {
        return {
            keys,
            updatedAt: String(row?.updated_at || '').trim() || null,
    ...
    const legacy = await getLegacySystemGeminiApiKeys(db)
...
async function setSystemGeminiApiKeys(db: D1Database, rawKeys: unknown): Promise<void> {
    await ensureNamespaceSettingsTable(db)
    const keys = normalizeGeminiApiKeyList(rawKeys)
    if (keys.length === 0) {
        await db.prepare(
            'DELETE FROM namespace_settings WHERE namespace_id = ? AND key = ?'
        ).bind(SYSTEM_GEMINI_NAMESPACE_ID, NS_SETTING_GEMINI_API_KEYS).run()
        await clearLegacySystemGeminiApiKeys(db)
        return
    }
    ...
    await db.prepare(
        ...
        SYSTEM_GEMINI_NAMESPACE_ID,
        NS_SETTING_GEMINI_API_KEYS,
        JSON.stringify(keys),
    ).run()
}

async function resolveNamespaceGeminiApiKeys(db: D1Database, _namespaceId: string): Promise<string[]> {
    const system = await getSystemGeminiApiKeyEntries(db)
    return system.keys
}

async function resolveNamespaceGeminiApiKey(db: D1Database, namespaceId: string): Promise<string> {
    const keys = await resolveNamespaceGeminiApiKeys(db, namespaceId)
    return keys[0] || ''
}

async function getNamespaceGeminiApiKeySettings(db: D1Database, _namespaceId: string) {
    const system = await getSystemGeminiApiKeyEntries(db)
    return {
        has_key: system.keys.length > 0,
        keys_count: system.keys.length,
        masked_key: maskApiKeyForDisplay(system.keys[0] || ''),
        masked_keys: system.keys.map((key) => maskApiKeyForDisplay(key)),
...
function normalizeCoverTemplateId(rawValue: string): string {
    const normalized = String(rawValue || '').trim().toLowerCase()
    return COVER_TEMPLATE_IDS.has(normalized) ? normalized : DEFAULT_COVER_TEMPLATE_ID
}

async function getNamespaceCoverTemplateEntry(db: D1Database, namespaceId: string): Promise<{ templateId: string; updatedAt: string | null }> {
    await ensureNamespaceSettingsTable(db)
    const row = await db.prepare(
        'SELECT value, updated_at FROM namespace_settings WHERE namespace_id = ? AND key = ?'
    ).bind(namespaceId, NS_SETTING_COVER_TEMPLATE).first() as { value?: string; updated_at?: string } | null
    return {
        templateId: normalizeCoverTemplateId(String(row?.value || '')),
        updatedAt: String(row?.updated_at || '').trim() || null,
    }
}
...
function normalizeNamespacePostingOrder(rawValue: string): NamespacePostingOrder {
    const normalized = String(rawValue || '').trim().toLowerCase()
    if (normalized === 'newest_first') return 'newest_first'
    if (normalized === 'random') return 'random'
    return 'oldest_first'
}

async function getNamespacePostingOrderEntry(db: D1Database, namespaceId: string): Promise<{ postingOrder: NamespacePostingOrder; updatedAt: string | null }> {
    await ensureNamespaceSettingsTable(db)
    const row = await db.prepare(
        'SELECT value, updated_at FROM namespace_settings WHERE namespace_id = ? AND key = ?'
    ).bind(namespaceId, NS_SETTING_POSTING_ORDER).first() as { value?: string; updated_at?: string } | null
    return {
        postingOrder: normalizeNamespacePostingOrder(String(row?.value || DEFAULT_POSTING_ORDER)),
        updatedAt: String(row?.updated_at || '').trim() || null,
    }
}

async function setNamespacePostingOrder(db: D1Database, namespaceId: string, rawPostingOrder: string): Promise<void> {
    await ensureNamespaceSettingsTable(db)
    const postingOrder = normalizeNamespacePostingOrder(rawPostingOrder)
    await db.prepare(
        `INSERT INTO namespace_settings (namespace_id, key, value, created_at, updated_at)
         VALUES (?, ?, ?, datetime('now'), datetime('now'))
         ON CONFLICT(namespace_id, key)
         DO UPDATE SET value = excluded.value, updated_at = datetime('now')`
    ).bind(namespaceId, NS_SETTING_POSTING_ORDER, postingOrder).run()
}

async function getNamespacePostingOrderSettings(db: D1Database, namespaceId: string) {
    const workspace = await getNamespacePostingOrderEntry(db, namespaceId)
    return {
...
function computeLazadaMemberMatchValue(expectedMemberId: string, actualMemberId?: string | null): number | null {
    const expected = normalizeLazadaMemberId(expectedMemberId)
    if (!expected) return null
    const actual = normalizeLazadaMemberId(actualMemberId || '')
    return actual === expected ? 1 : 0
}
...
function extractOriginalLinkFromHtml(html: string): string {
...
function buildAffiliateResolveHeaders(): HeadersInit {
    return {
...
async function resolveOriginalAffiliateLink(rawLink: string, logPrefix: string): Promise<string> {
    ...
    try {
        const resp = await fetchWithTimeout(input, {
            method: 'GET',
            redirect: 'follow',
            headers: buildAffiliateResolveHeaders(),
        }, 15000, `${logPrefix}_resolve_original`)
...
async function resolveAffiliateRedirectLink(rawLink: string, logPrefix: string): Promise<string> {
    const input = String(rawLink || '').trim()
    if (!input) return ''
    try {
        const resp = await fetchWithTimeout(input, {
            ...
            redirect: 'follow',
            headers: buildAffiliateResolveHeaders(),
        }, 15000, `${logPrefix}_resolve_redirect`)
        return String(resp.url || '').trim() || input
    } catch {
        return input
    }
}

async function resolveAffiliateTrackingLink(rawLink: string, logPrefix: string): Promise<string> {
    const current = String(rawLink || '').trim()
    if (!current) return ''
    const redirectedFirst = await resolveAffiliateRedirectLink(current, logPrefix)
    if (redirectedFirst && redirectedFirst !== current) {
        return redirectedFirst
    }
    const extracted = await resolveOriginalAffiliateLink(current, logPrefix)
    const candidate = extracted || current
    const redirected = await resolveAffiliateRedirectLink(candidate, logPrefix)
...
function getVideoSourceShopeeLink(video: Record<string, unknown>): string {
    return pickFirstShopeeUrl(
...
function getVideoSourceLazadaLink(video: Record<string, unknown>): string {
    return pickFirstLazadaUrl(
...
function extractLazadaTrackingSourceFromLink(link: string): string {
    const rawLink = pickFirstLazadaUrl(link) || String(link || '').trim()
    if (!rawLink) return ''
...
function extractLazadaMemberIdFromLink(link: string): string {
...
function isLikelyConvertedShopeeLink(link: string, expectedUtmId = ''): boolean {
    ...
    try {
        ...
        // ignore invalid URL parsing here and continue to UTM-based check
    }

    const expected = normalizeShortlinkExpectedUtmId(expectedUtmId)
    const actual = normalizeShortlinkExpectedUtmId(extractShopeeUtmSourceFromLink(rawLink))
    return !!expected && !!actual && expected === actual
}

function isLikelyConvertedLazadaLink(link: string): boolean {
    const rawLink = pickFirstLazadaUrl(link) || String(link || '').trim()
    if (!rawLink) return false
...
function getVideoAffiliateConversionState(
    ...
    return {
        ...
        missingShopeeSource,
        missingLazadaSource,
        needsShopeeConversion,
        needsLazadaConversion,
        awaitingConversion,
        galleryReady,
    }
}

function isVideoAwaitingAffiliateConversion(video: Record<string, unknown>, expectedUtmId = '', expectedLazadaMemberId = ''): boolean {
    return getVideoAffiliateConversionState(video, expectedUtmId, expectedLazadaMemberId).awaitingConversion
}

async function listVideosAwaitingAffiliateConversion(env: Env, namespaceId: string, options: { systemWide?: boolean } = {}): Promise<Array<Record<string, unknown>>> {
    void options
    const normalizedNamespaceId = String(namespaceId || '').trim()
    if (!normalizedNamespaceId) return []
    ...
    const inventory = await getNamespaceGalleryInventory(env, normalizedNamespaceId)
    return inventory.videos.filter((video) => {
        const record = video as Record<string, unknown>
        return isVideoAwaitingAffiliateConversion(
            record,
            String(record.shortlink_expected_utm_id || '').trim(),
            String(record.lazada_expected_member_id || '').trim(),
        )
    })
}

async function persistAffiliateVideoMeta(env: Env, namespaceId: string, videoId: string, meta: Record<string, unknown>): Promise<void> {
    const bucket = new BotBucket(env.BUCKET, namespaceId) as unknown as R2Bucket
    await bucket.put(`videos/${videoId}.json`, JSON.stringify(meta, null, 2), {
        httpMetadata: { contentType: 'application/json' },
    })
    await updateGalleryCache(bucket, videoId)
    await syncGalleryIndexEntry(env, namespaceId, videoId).catch((error) => {
        console.log(`[AFFILIATE-CONVERT] sync gallery index failed ns=${namespaceId} video=${videoId}: ${error instanceof Error ? error.message : String(error)}`)
    })
    await env.BUCKET.delete('_admin_cache/all_gallery_videos.json').catch(() => { })
    await env.BUCKET.delete('_admin_cache/all_gallery_owner_videos.json').catch(() => { })
}
...
async function processPendingAffiliateConversions(env: Env): Promise<void> {
    ...
    for (const namespaceId of namespaceIds) {
        ...
        const bucket = new BotBucket(env.BUCKET, namespaceId) as unknown as R2Bucket
        const expectedUtmId = await resolveNamespaceShopeeShortlinkExpectedUtmId(env.DB, namespaceId).catch(() => '')
        const expectedLazadaMemberId = await resolveNamespaceLazadaExpectedMemberId(env.DB, namespaceId).catch(() => '')

        for (const pendingVideo of pendingVideos) {
            const videoId = String(pendingVideo.id || '').trim()
            if (!videoId) continue
            ...
            if (conversionState.hasShopeeSource) {
                ...
                if (sourceShopeeLink) {
                    ...
                    const resolvedShopeeLink = await shortenShopeeLinkForNamespace({
                        env,
                        namespaceId,
                        shopeeLink: sourceShopeeLink,
                        logPrefix: `AFFILIATE-CONVERT ${namespaceId}/${videoId} SHOPEE`,
                        trace,
                    })
                    const effectiveShopeeLink = resolvedShopeeLink || conversionState.shopeeCurrentLink || sourceShopeeLink
                    nextMeta.shopeeLink = effectiveShopeeLink
                    nextStateFields.shopee_link = effectiveShopeeLink
                    const shopeeConvertedReady = !!effectiveShopeeLink
                        && effectiveShopeeLink !== sourceShopeeLink
                        && trace.status === 'shortened'
                        && (
                            !expectedUtmId
                            || normalizeShortlinkExpectedUtmId(String(trace.utmSource || '')) === normalizeShortlinkExpectedUtmId(expectedUtmId)
                        )
                    if (shopeeConvertedReady) {
            ...
            if (conversionState.hasLazadaSource) {
                ...
                if (sourceLazadaLink) {
                    ...
                    const trace: { utmSource?: string | null; memberId?: string | null; status?: 'disabled' | 'shortened' | 'fallback'; error?: string | null } = {}
                    const resolvedLazadaLink = await shortenLazadaLinkForNamespace({
                        env,
                        namespaceId,
                        lazadaLink: sourceLazadaLink,
                        logPrefix: `AFFILIATE-CONVERT ${namespaceId}/${videoId} LAZADA`,
                        trace,
                    })
                    const effectiveLazadaLink = resolvedLazadaLink || conversionState.lazadaCurrentLink || sourceLazadaLink
                    const nextMemberId = normalizeLazadaMemberId(String(
                        trace.memberId ||
                        pendingVideo.lazadaMemberId ||
                        pendingVideo.lazada_member_id ||
                        nextMeta.lazadaMemberId ||
                        nextMeta.lazada_member_id ||
                        ''
                    ))
                    nextMeta.lazadaLink = effectiveLazadaLink
                    nextStateFields.lazada_link = effectiveLazadaLink
                    if (nextMemberId) {
                        nextMeta.lazadaMemberId = nextMemberId
                        nextStateFields.lazada_member_id = nextMemberId
                    }
                    const lazadaConvertedReady = !!effectiveLazadaLink
            ...
            await persistAffiliateVideoMeta(env, namespaceId, videoId, nextMeta)
            await upsertNamespaceVideoState(env.DB, namespaceId, videoId, nextStateFields)
        }
    }
}

async function autoImportAndProcessForAdmin(env: Env, ctx?: ExecutionContext): Promise<void> {
    // 1. Find primary admin namespace
    ...
    try {
        const { results } = await env.DB.prepare('SELECT DISTINCT namespace_id FROM email_namespaces').all()
        for (const row of results || []) {
            const namespaceId = String((row as any)?.namespace_id || '').trim()
            if (namespaceId) namespaceIds.add(namespaceId)
        }
    } catch { /* best effort */ }

    for (const sourceNamespaceId of namespaceIds) {
        if (!sourceNamespaceId || sourceNamespaceId === adminNamespaceId) continue

        const sourceBucket = new BotBucket(env.BUCKET, sourceNamespaceId) as unknown as R2Bucket
        const [sourceVideos, waitingVideos] = await Promise.all([
            listNamespaceOriginalArchiveVideos({
                env,
                bucket: sourceBucket,
                namespaceId: sourceNamespaceId,
            }).catch(() => []),
            listReadyLineWaitingVideoStates(sourceBucket).catch(() => []),
        ])
        const sourceVideoMap = new Map<string, Record<string, unknown>>()
        for (const sourceVideo of sourceVideos) {
            const videoId = String(sourceVideo.id || '').trim()
            if (!videoId) continue
            sourceVideoMap.set(videoId, sourceVideo)
        }
        for (const waitingState of waitingVideos) {
            const videoId = String(waitingState.id || '').trim()
            if (!videoId) continue
            sourceVideoMap.set(videoId, {
                id: videoId,
                videoUrl: waitingState.videoUrl,
                createdAt: waitingState.createdAt,
                sourceType: waitingState.sourceType || 'line_video',
        ...
        for (const sourceVideo of sourceVideoMap.values()) {
            const videoId = String(sourceVideo.id || '').trim()
            if (!videoId) continue

            const [existingGallery, existingQueue, existingProcessingStatus, existingInbox, existingMetaObj] = await Promise.all([
                adminBucket.head(`videos/${videoId}.json`).catch(() => null),
                adminBucket.head(`_queue/${videoId}.json`).catch(() => null),
                getProcessingJobStatus(adminBucket, videoId).catch(() => ''),
                getInboxVideoRecord(adminBucket, videoId).catch(() => null),
                adminBucket.get(`videos/${videoId}.json`).catch(() => null),
            ])
            const hasExistingActiveProcessing = existingProcessingStatus && existingProcessingStatus !== 'failed'
            if (existingQueue || hasExistingActiveProcessing) continue
            if (existingInbox) continue

            let hasReusableProcessedCopy = !!existingGallery
            if (existingMetaObj) {
                const existingMeta = await existingMetaObj.json().catch(() => null) as Record<string, unknown> | null
                if (existingMeta && isImportedVideoMetaPointingAtSourceNamespace({
                    meta: existingMeta,
                    targetNamespaceId: adminNamespaceId,
                    sourceNamespaceId,
                })) {
                    await clearNamespaceProcessedArtifacts(adminBucket, videoId)
                    hasReusableProcessedCopy = false
                }
            }
            if (hasReusableProcessedCopy) continue

            const synced = await syncImportedOriginalIntoNamespace({
                env,
                targetNamespaceId: adminNamespaceId,
                targetBucket: adminBucket,
                sourceNamespaceId,
...
async function resolveNamespaceAffiliateShortlinkAccount(db: D1Database, namespaceId: string): Promise<string> {
    ...
    const extracted = extractShortlinkAccountFromBaseUrl(shopeeRow.baseUrl) || extractShortlinkAccountFromBaseUrl(lazadaRow.baseUrl) || ''
    if (extracted) return extracted
...
function isPostRoleToken(token: string): boolean {
    // Post role = page-scoped tokens (non-EAAD6). These are now preferred for
    // both reel posting and comment resolution, with EAAD6 kept as legacy fallback.
...
function normalizeDirectVideoTokenPool(tokens: string[]): string[] {
    // Direct video posting/comment now runs with page-scoped tokens only.
    return normalizeCommentTokenPool(tokens)
}

function buildPreferredCommentTokenPool(params: {
    primaryToken?: string | null
    postTokens?: string[]
    commentTokens?: string[]
}): string[] {
    return normalizeDirectVideoTokenPool([
        String(params.primaryToken || '').trim(),
        ...normalizePostTokenPool(params.postTokens || []),
        ...normalizeCommentTokenPool(params.commentTokens || []),
    ])
}

function buildPrimaryPostingTokenCandidates(params: {
    primaryToken?: string | null
    postTokens?: string[]
    commentTokens?: string[]
}): string[] {
...
async function rebuildTaggedPageProfileTokens(env: Env, namespaceId: string, pageId: string, pageName: string): Promise<{ primaryToken: string; tokens: string[]; postTokens: string[]; commentTokens: string[] }> {
    ...
    const rebuiltCommentTokens = buildPreferredCommentTokenPool({
...
async function getPageTokenCandidates(params: {
    ...
    const commentTokens = buildPreferredCommentTokenPool({
        primaryToken,
        postTokens,
        commentTokens: storedCommentTokens,
    })
...
async function initReelUploadWithPostingTokenAutoRecover(params: {
    ...
    return initReelUploadWithPostingTokenFallback({
...
function deriveLineSubmissionActorId(lineUserId: string): string {
    return String(lineUserId || '').replace(/\D/g, '').slice(0, 15)
}

function deriveLineSubmissionChatId(lineUserId: string): number {
    return Number(deriveLineSubmissionActorId(lineUserId)) || 1
}
...
async function resolveSessionSubmissionChatId(db: D1Database, emailRaw: string): Promise<number> {
    const email = String(emailRaw || '').trim().toLowerCase()
    if (!email) return 1

    const lineRow = await db.prepare(
        `SELECT line_user_id
         FROM line_users
         WHERE lower(trim(email)) = ?
         ORDER BY datetime(updated_at) DESC, datetime(created_at) DESC
         LIMIT 1`
    ).bind(email).first().catch(() => null) as { line_user_id?: string } | null
    const lineUserId = String(lineRow?.line_user_id || '').trim()
    if (lineUserId) {
        return deriveLineSubmissionChatId(lineUserId)
    }

    const digits = email.replace(/\D/g, '').slice(0, 15)
    if (digits) {
        return Number(digits) || 1
    }

    return hashStringToPositiveInt(email)
}
...
async function getExistingLinkSubmissionVideoIds(params: {
    db: D1Database
    namespaceId: string
    videoIds: string[]
}): Promise<Set<string>> {
    const namespaceId = String(params.namespaceId || '').trim()
    const videoIds = Array.from(new Set((params.videoIds || []).map((id) => String(id || '').trim()).filter(Boolean)))
    if (!namespaceId || !videoIds.length) return new Set()
    ...
    for (let index = 0; index < videoIds.length; index += chunkSize) {
        ...
        ).bind(namespaceId, ...chunk).all() as { results?: Array<{ video_id?: string }> }
        for (const row of rows.results || []) {
            const videoId = String(row?.video_id || '').trim()
...
async function backfillRecentInboxLinkSubmissions(params: {
    db: D1Database
    bucket: R2Bucket
    namespaceId: string
    days?: number
    ...
    const days = Number.isFinite(Number(params.days))
        ? Math.max(1, Math.min(30, Math.floor(Number(params.days))))
        : 7
    const limit = Number.isFinite(Number(params.limit))
        ? Math.max(1, Math.min(500, Math.floor(Number(params.limit))))
        : 200
    const cutoffMs = Date.now() - (days * 24 * 60 * 60 * 1000)

    const inboxRecords = await listInboxVideoRecords(params.bucket).catch(() => [])
    const candidates = inboxRecords
        .filter((item) => {
            if (item.status !== 'ready') return false
            if (Number(item.chatId || 0) <= 0) return false
            if (!String(item.shopeeLink || '').trim()) return false
            const submissionAt = Date.parse(String(item.linkRecordedAt || item.updatedAt || item.createdAt || '').trim())
            return Number.isFinite(submissionAt) && submissionAt >= cutoffMs
        })
        .sort((a, b) => {
            const aMs = Date.parse(String(a.linkRecordedAt || a.updatedAt || a.createdAt || '').trim()) || 0
            const bMs = Date.parse(String(b.linkRecordedAt || b.updatedAt || b.createdAt || '').trim()) || 0
            return bMs - aMs
        })
        .slice(0, limit)
    ...
    const existingVideoIds = await getExistingLinkSubmissionVideoIds({
        db: params.db,
        namespaceId,
        videoIds: candidates.map((item) => item.id),
    })

    let inserted = 0
    for (const item of candidates) {
        if (existingVideoIds.has(item.id)) continue
        const createdAt = String(item.linkRecordedAt || item.updatedAt || item.createdAt || '').trim() || new Date().toISOString()
        await recordLinkSubmission({
            db: params.db,
            namespaceId,
            telegramId: String(item.chatId),
            videoId: item.id,
            shopeeLink: String(item.shopeeLink || '').trim(),
            createdAt,
        })
...
async function shortenLazadaLinkForNamespace(params: {
    ...
    const writeTrace = (payload: { utmSource?: string | null; memberId?: string | null; status?: 'disabled' | 'shortened' | 'fallback'; error?: string | null }) => {
        if (!params.trace) return
        if (payload.utmSource !== undefined) params.trace.utmSource = payload.utmSource
        if (payload.memberId !== undefined) params.trace.memberId = payload.memberId
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        try {
            ...
            const utmSource = String(data.utm_source || data.utmSource || '').trim() || extractLazadaTrackingSourceFromLink(shortLink) || null
            ...
            lastError = msg
            console.log(`[${params.logPrefix}] Lazada shortlink attempt ${attempt}/${maxAttempts} failed: ${msg}`)
...
async function verifyAffiliateLinksForPosting(params: {
    ...
    const shopee: PostingAffiliatePlatformVerification = {
        ...
        resolvedLink: '',
        expectedId: expectedShopeeId,
        actualId: '',
        status: !shopeeInputLink ? 'missing_link' : !expectedShopeeId ? 'skipped' : 'error',
        match: computeShortlinkUtmMatchValue(expectedShopeeId, null),
        error: null,
    }
    ...
    if (shopeeInputLink && expectedShopeeId) {
        try {
            ...
            shopee.status = 'error'
            shopee.match = 0
            shopee.error = `Shopee verify failed: ${e instanceof Error ? e.message : String(e)}`
        }
    }

    const lazada: PostingAffiliatePlatformVerification = {
        inputLink: lazadaInputLink,
        resolvedLink: '',
...
async function updatePostHistoryAffiliateVerificationById(db: D1Database, historyId: number, verification: PostingAffiliateVerificationResult): Promise<void> {
    ...
    await db.prepare(
        `UPDATE post_history
         SET shopee_link = ?,
             lazada_link = ?,
             shortlink_utm_source = ?,
             shortlink_status = ?,
             shortlink_error = ?,
             shortlink_expected_utm_id = ?,
             shortlink_utm_match = ?,
             lazada_member_id = ?,
        ...
        verification.shopee.inputLink || null,
        verification.lazada.inputLink || null,
        verification.shopee.actualId ? `an_${verification.shopee.actualId}` : null,
        verification.status,
        verification.error,
        verification.shopee.expectedId || null,
        verification.shopee.match,
        verification.lazada.actualId || null,
        verification.lazada.expectedId || null,
        verification.lazada.match,
        historyId,
    ).run()
}

async function updatePostHistoryAffiliateVerificationByPostingKey(
    db: D1Database,
    pageId: string,
    videoId: string,
    verification: PostingAffiliateVerificationResult,
    botId?: string,
): Promise<void> {
    const normalizedPageId = String(pageId || '').trim()
    const normalizedVideoId = String(videoId || '').trim()
    ...
    const sql = normalizedBotId
        ? `UPDATE post_history
           SET shopee_link = ?,
               ...
               lazada_expected_member_id = ?,
               lazada_member_match = ?
           WHERE page_id = ? AND video_id = ? AND status = 'posting' AND bot_id = ?`
        : `UPDATE post_history
           SET shopee_link = ?,
               lazada_link = ?,
               shortlink_utm_source = ?,
               ...
               lazada_expected_member_id = ?,
               lazada_member_match = ?
           WHERE page_id = ? AND video_id = ? AND status = 'posting'`
    ...
    if (normalizedBotId) bindings.push(normalizedBotId)
...
async function autoSyncPagesForNamespace(
    ...
    if (hasTagSelector) {
        try {
            ...
            for (const pageId of allResolvedPageIds) {
                ...
                const commentTokens = buildPreferredCommentTokenPool({
                    postTokens,
                    commentTokens: commentRecords.map((r) => String(r.access_token || '').trim()),
                })
            ...
            if (pageMap.size > 0) {
                ...
                for (const page of pageMap.values()) {
                    ...
                    const discoveredCommentTokenPool = buildPreferredCommentTokenPool({
                        primaryToken: page.access_token,
                        postTokens: postTokenPoolByPage.get(pageId) || [],
                        commentTokens: [
                    ...
                    const commentTokenPool = discoveredCommentTokenPool.length > 0
                        ? buildPreferredCommentTokenPool({
                            primaryToken,
                            postTokens: postTokenPool,
                            commentTokens: [
                        ...
                        : buildPreferredCommentTokenPool({
                            primaryToken,
                            postTokens: postTokenPool,
                            commentTokens: existingPool[pageId]?.comment_tokens || [],
                        })
    ...
    for (const page of pageMap.values()) {
        ...
        // Unified: merge ALL tokens (comment + post) into comment pool
        const commentTokenPool = buildPreferredCommentTokenPool({
            primaryToken: page.access_token,
            postTokens: existingEntry.post_tokens || [],
            commentTokens: [
                commentTokenRaw,
                ...(existingEntry.comment_tokens || []),
            ],
    ...
    if (!postHistoryTraceColumnsReady) {
        postHistoryTraceColumnsReady = (async () => {
            const statements = [
                ...
                'ALTER TABLE post_history ADD COLUMN lazada_expected_member_id TEXT',
...
async function buildPostingCaptionFromMeta(
    meta: Record<string, unknown>,
    apiKeys: string[],
    model: string,
): Promise<string> {
    ...
    let caption = title
        ? title
        : script
            ? await generateCaption(script, apiKeys, model)
            : 'AI Dubbed Video'
    caption += `\n#สินค้า #ของน่าใช้ #ช็อปปิ้งออนไลน์${category ? ` #${category}` : ''}`
    return caption
}
...
async function loadPostingThumbnailAsset(params: {
    ...
    namespaceId: string
    videoId: string
}): Promise<{ buffer: ArrayBuffer; contentType: string; key: string } | null> {
    const namespaceId = String(params.namespaceId || '').trim()
    const videoId = String(params.videoId || '').trim()
    ...
    for (const candidate of candidates) {
        const obj = await bucket.get(candidate.key).catch(() => null)
        if (!obj) continue
        const buffer = await obj.arrayBuffer().catch(() => null)
        if (!buffer || buffer.byteLength === 0) continue
        return {
            buffer,
            contentType: String(obj.httpMetadata?.contentType || candidate.fallbackContentType).trim() || candidate.fallbackContentType,
            key: candidate.key,
        }
    }
...
async function loadLatestVideoMetaForPosting(
    bucket: R2Bucket,
    videoIdRaw: string,
    fallbackMeta: Record<string, unknown>,
): Promise<Record<string, unknown>> {
    const videoId = String(videoIdRaw || '').trim()
    if (!videoId) return { ...(fallbackMeta || {}) }

    const metaObj = await bucket.get(`videos/${videoId}.json`).catch(() => null)
    if (!metaObj) return { ...(fallbackMeta || {}) }

    try {
        const latestMeta = await metaObj.json() as Record<string, unknown>
        return {
            ...(fallbackMeta || {}),
            ...(latestMeta || {}),
        }
...
app.get('/api/dashboard', async (c) => {
    try {
        ...
        const [postsAllRow, postsOnDateRow, linksAllRow, linksOnDateRow, usersRes, scopedSessionsRes, lineUsersRes, linksByAdminRes] = await Promise.all([
            ...
            c.env.DB.prepare(
                'SELECT line_user_id, display_name, picture_url, email FROM line_users WHERE namespace_id = ?'
            ).bind(botId).all() as Promise<{ results?: Array<{ line_user_id?: string; display_name?: string; picture_url?: string; email?: string }> }>,
            c.env.DB.prepare(
        ...
        const lineProfileByActorId = new Map<string, {
            line_user_id: string
            display_name: string
            picture_url: string
            email: string
        }>()
        for (const row of lineUsersRes.results || []) {
            const lineUserId = String(row?.line_user_id || '').trim()
            const actorId = deriveLineSubmissionActorId(lineUserId)
            if (!lineUserId || !actorId || lineProfileByActorId.has(actorId)) continue
            lineProfileByActorId.set(actorId, {
                line_user_id: lineUserId,
                display_name: String(row?.display_name || '').trim(),
                picture_url: String(row?.picture_url || '').trim(),
        ...
        for (const row of linksByAdminRes.results || []) {
            ...
            const lineProfile = telegramId ? lineProfileByActorId.get(telegramId) : undefined
            const email = lineProfile?.email || (telegramId ? (emailByTelegram.get(telegramId) || `telegram:${telegramId}`) : 'unknown')
            const displayName = String(lineProfile?.display_name || '').trim() || email

            adminMap.set(key, {
                telegram_id: telegramId || '-',
                email,
                display_name: displayName,
                picture_url: String(lineProfile?.picture_url || '').trim(),
                line_user_id: String(lineProfile?.line_user_id || '').trim(),
                links: count,
            })
        }
...
app.get('/api/pages/:id/tag-profiles', async (c) => {
    ...
    try {
        ...
        try {
            ...
            const discoveredCommentTokens = buildPreferredCommentTokenPool({
                primaryToken: storedAccessScoped,
                postTokens: discoveredPostTokens,
                commentTokens: resolvedItems
    ...
    try {
        ...
        if (access_token !== undefined) {
            ...
            if (rawToken) {
                ...
                const resolvedAccessToken = String(accessResolved.token || '').trim()
                if (!resolvedAccessToken) {
                    return c.json({
                        error: 'invalid_access_token',
                        details: String(accessResolved.reason || 'page_not_found_in_me_accounts'),
                    }, 400)
                }

                await c.env.DB.prepare(
                    'UPDATE pages SET access_token = ?, updated_at = datetime("now") WHERE id = ? AND bot_id = ?'
                ).bind(resolvedAccessToken, id, c.get('botId')).run()
                effectivePostToken = resolvedAccessToken
                nextPostTokenPool = normalizePostTokenPool([
                    resolvedAccessToken,
                    ...nextPostTokenPool,
                ])
                nextCommentTokenPool = buildPreferredCommentTokenPool({
                    primaryToken: resolvedAccessToken,
                    postTokens: nextPostTokenPool,
                    commentTokens: nextCommentTokenPool,
                })
        ...
        if (comment_token !== undefined || access_token !== undefined) {
            const rawCommentToken = String(comment_token ?? access_token ?? '').trim()
            if (rawCommentToken) {
                const resolvedCommentToken = await resolveCommentPageTokenForPage(c.env, id, rawCommentToken)
                nextCommentTokenPool = buildPreferredCommentTokenPool({
                    primaryToken: effectivePostToken,
                    postTokens: nextPostTokenPool,
                    commentTokens: [
                        resolvedCommentToken,
                        ...nextCommentTokenPool,
            ...
            } else if (comment_token !== undefined) {
                nextCommentTokenPool = []
                resolved.comment_token = 'provided_as_is'
            }
        }
...
app.post('/api/pages/import', async (c) => {
    try {
        const body = await c.req.json()
        const { user_token, post_user_token, comment_user_token } = body || {}
        const botId = c.get('botId')
        const postRawInput = String(post_user_token || user_token || '').trim()
        const commentRawInput = String(comment_user_token || postRawInput).trim()

        if (!postRawInput) {
            return c.json({ error: 'User token is required' }, 400)
        }

        const postImportToken = postRawInput
        const commentImportToken = commentRawInput || postImportToken
        ...
        for (const fbPage of fbPages) {
            ...
            tokenPool[pageId] = {
                post_tokens: nextPostTokens,
                comment_tokens: buildPreferredCommentTokenPool({
                    primaryToken: pageAccessToken,
                    postTokens: nextPostTokens,
                    commentTokens: [
                        pageCommentTokenRaw,
                        ...(existingEntry.comment_tokens || []),
                    ],
        ...
        return c.json({
            ...
            source: 'provided_as_is',
            source_comment: 'provided_as_is',
...
app.get('/api/post-history', async (c) => {
    try {
        const botId = c.get('botId')
        const dateFilter = String(c.req.query('date') || '').trim()
        const limitParam = Number(c.req.query('limit') || 100)
        const limit = Math.max(1, Math.min(200, Number.isFinite(limitParam) ? Math.floor(limitParam) : 100))
        const hasDateFilter = /^\d{4}-\d{2}-\d{2}$/.test(dateFilter)
        const { results } = await c.env.DB.prepare(
            `WITH ranked_history AS (
                 ...
                 WHERE ph.status != 'deleted' AND p.bot_id = ?
                   AND (? = '' OR date(datetime(ph.posted_at, '+7 hours')) = ?)
             )
             SELECT *
             FROM ranked_history
             WHERE rn = 1
             ...
             LIMIT ?`
        ).bind(botId, hasDateFilter ? dateFilter : '', hasDateFilter ? dateFilter : '', limit).all()
...
app.post('/api/post-history/:id/retry-post', async (c) => {
    ...
    try {
        ...
        if (!sourceRow?.page_id || !sourceRow?.video_id) {
            return c.json({ error: 'History not found' }, 404)
        }

        pageId = String(sourceRow.page_id || '').trim()
        ...
        const activePosting = await env.DB.prepare(
            `SELECT id
             ...
             ORDER BY posted_at DESC, id DESC
             LIMIT 1`
        ).bind(botId, pageId).first() as { id?: number } | null
        if (typeof activePosting?.id === 'number') {
            return c.json({
                error: 'page_already_posting',
                details: { history_id: activePosting.id },
            }, 409)
        }
        ...
        if (!videoRef) {
            return c.json({ error: 'Video metadata not found for repost' }, 404)
        }
        selectedVideoNamespaceId = String(videoRef.sourceNamespaceId || botId).trim() || botId
        ...
        const primaryPostingTokenCandidates = buildPrimaryPostingTokenCandidates({
            primaryToken: pageAccessToken,
        ...
        await ensurePostHistoryTraceColumns(env.DB)
        const commentTokenHint = deriveCommentTokenHint(pageCommentToken)
        ...
        const inserted = await env.DB.prepare(
            ...
            initialCommentState.status,
            commentTokenHint,
            initialCommentState.error,
            null,
        ...
        if (retryHistoryId) {
            await updatePostHistoryAffiliateVerificationById(env.DB, retryHistoryId, affiliateVerification)
        }
        if (!affiliateVerification.ok) {
            if (retryHistoryId) {
                ...
                ).bind(affiliateVerification.error || 'affiliate_verification_failed', retryHistoryId).run().catch(() => { })
            }
            return c.json({
        ...
        if (pageAccessToken && pageId && selectedVideoId && retryHistoryId) {
            try {
                let recoveredPostId = ''
                ...
                if (!recoveredPostId && selectedCaption) {
                    const feedRecovered = await recoverPublishedReelFromRecentFeed({
                        ...
                        logPrefix: 'RETRY-POST-RECOVER-FEED',
                    })
                    if (feedRecovered.published && feedRecovered.post_id) {
                ...
                if (recoveredPostId) {
                    ...
                    if (normalizedShopeeLink && !skipComment && hasCommentToken) {
                        ...
                        if (!commentResult.ok) {
                            ...
                            await notifyCommentTokenIssue(
                                ...
                                console.error(`[RETRY-POST-RECOVER] notifyCommentTokenIssue failed: ${notifyErr instanceof Error ? notifyErr.message : String(notifyErr)}`)
                            })
                        } else {
                            commentStatus = 'success'
                            commentFbId = commentResult.id || null
                    ...
                    const sourceBucket = selectedVideoNamespaceId === botId
                        ? c.get('bucket')
                        : new BotBucket(env.BUCKET, selectedVideoNamespaceId || botId) as unknown as R2Bucket
                    await clearVideoShopeeLink(sourceBucket, selectedVideoId)
                    return c.json({
                        success: true,
                        recovered: true,
                        ...
                        fb_reel_url: recoveredReelUrl || `https://www.facebook.com/reel/${recoveredPostId}`,
                    })
                }
            } catch (recoverErr) {
                console.error(`[RETRY-POST-RECOVER] failed: ${recoverErr instanceof Error ? recoverErr.message : String(recoverErr)}`)
            }
        }
        ...
        if (retryHistoryId) {
            await env.DB.prepare(
                "UPDATE post_history SET status = 'failed', error_message = ?, comment_status = 'not_attempted' WHERE id = ? AND status = 'posting'"
            ).bind(errorMsg, retryHistoryId).run().catch(() => { })
        }
...
async function generateCaption(script: string, apiKeys: string[], model: string): Promise<string> {
...
app.post('/api/generate-title/:id', async (c) => {
    const env = c.env
    const id = c.req.param('id')
    const apiKeys = await resolveNamespaceGeminiApiKeys(env.DB, String(c.get('botId') || ''))
    if (apiKeys.length === 0) return c.json({ error: 'Gemini API key กลางของระบบยังไม่ได้ตั้งค่า' }, 400)
    const model = env.GEMINI_MODEL || 'gemini-3-flash-preview'
...
script: ${script}`
        ...
        if (!resp.ok) continue
        const result = await resp.json() as { candidates?: Array<{ content?: { parts?: Array<{ text?: string }> } }> }
        title = result?.candidates?.[0]?.content?.parts?.[0]?.text?.trim() || ''
        if (title) break
    }
...
app.post('/api/pages/:id/force-post', async (c) => {
    ...
    try {
        ...
        fallbackPostTokenCandidates = tokenCandidates.postTokens.length > 0
            ? tokenCandidates.postTokens
            : normalizePostTokenPool([pageAccessToken])
        primaryPostingTokenCandidates = buildPrimaryPostingTokenCandidates({
            primaryToken: pageAccessToken,
            postTokens: fallbackPostTokenCandidates,
            commentTokens: commentTokenCandidates,
        })
    ...
    try {
        ...
        if (!affiliateVerification.ok) {
            return c.json({
                error: 'affiliate_verification_failed',
                details: affiliateVerification.error || 'Affiliate verification failed',
                shopee_expected_id: affiliateVerification.shopee.expectedId || null,
                shopee_actual_id: affiliateVerification.shopee.actualId || null,
                lazada_expected_member_id: affiliateVerification.lazada.expectedId || null,
                lazada_actual_member_id: affiliateVerification.lazada.actualId || null,
            }, 400)
        }
...
async function handleScheduled(env: Env, ctx?: ExecutionContext) {
    ...
    for (const page of pages) {
        ...
        if (!apiKeys) {
            apiKeys = await resolveNamespaceGeminiApiKeys(env.DB, botId)
            geminiApiKeysByNamespace.set(botId, apiKeys)
        }
        const hasManualCaption = !!normalizeManualCaption(meta.manualCaption || meta.caption || '')
        const title = metaToString(meta, 'title')
        const script = metaToString(meta, 'script')
        ...
        try {
            ...
            for (const obj of list.objects) {
                ...
                if (job.status === 'failed') {
                    ...
                    const failedAt = String(job.updatedAt || job.createdAt || '').trim() || obj.uploaded.toISOString()
                    const elapsedSinceFailure = now - new Date(failedAt).getTime()
                    if (elapsedSinceFailure < FAILED_RETRY_DELAY_MS) continue

                    const nextRetryCount = Number(job.retryCount || 0) + 1
                    if (!String(job.videoUrl || '').trim()) continue
                    if (nextRetryCount > MAX_RETRIES) continue

                    await botBucket.delete(obj.key)
                    await botBucket.put(`_queue/${job.id}.json`, JSON.stringify({
                        id: job.id,
                        videoUrl: job.videoUrl,
                        chatId: Number(job.chatId || 0),
                    ...
                    console.log(`[WATCHDOG] Failed job ${job.id} moved back to queue (retry #${nextRetryCount})`)
                    continue
                }
