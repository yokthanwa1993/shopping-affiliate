...
function loginHtmlPage(opts = {}) {
  ...
  const styles = [
    ...
    '.cred-item .actions{display:flex;align-items:center;gap:8px;flex:0 0 auto;}',
    '.cred-item button.reauth,.cred-item button.del{background:#fff;border:1px solid #d4d8df;padding:5px 10px;border-radius:6px;font-size:12px;font-family:inherit;cursor:pointer;transition:.15s ease;flex:0 0 auto;}',
    '.cred-item button.reauth{color:#1f6feb;}',
    '.cred-item button.reauth:hover:not(:disabled){background:#1f6feb;color:#fff;border-color:#1f6feb;}',
    '.cred-item button.del{color:#a4262c;}',
    '.cred-item button.del:hover:not(:disabled){background:#a4262c;color:#fff;border-color:#a4262c;}',
    '.cred-item button.reauth:disabled,.cred-item button.del:disabled{opacity:.55;cursor:not-allowed;}',
  ...
  const script = [
    ...
    'function buildReauthUrl(platform,account){',
    '  return "/login?json=1&platform="+encodeURIComponent(platform||"")+"&account="+encodeURIComponent(account||"")+"&autofill=1";',
    '}',
    ...
    '        var reauth=document.createElement("button");reauth.type="button";reauth.className="reauth";reauth.textContent="Re-auth บัญชีนี้";',
    '        reauth.setAttribute("aria-label","Re-auth "+platformLabel(it.platform)+" account "+(it.account||""));',
    '        reauth.addEventListener("click",function(){',
    '          reauth.disabled=true;setToast("info","กำลังเปิด Re-auth สำหรับ "+platformLabel(it.platform)+" / "+(it.account||"")+"…");',
    ...
    '            .then(function(res){',
    '              var j=res.json||{};var suffix=j.reason?(" ("+j.reason+")"):(j.status?(" ("+j.status+")"):"");',
    '              if(res.status>=200&&res.status<300){setToast("ok","เปิด Re-auth แล้วสำหรับ "+platformLabel(it.platform)+" / "+(it.account||"")+suffix);}',
    '              else{setToast("err","เปิด Re-auth ไม่สำเร็จ"+(j.error?": "+j.error:suffix));}',
    '            })',
    '            .catch(function(e){setToast("err","เปิด Re-auth ไม่สำเร็จ: "+(e&&e.message?e.message:String(e)));})',
    '            .then(function(){reauth.disabled=false;});',
    '        });',
