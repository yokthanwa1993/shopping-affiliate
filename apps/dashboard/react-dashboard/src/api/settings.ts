...
export const pageSettingsSchema = z.object({
  ...
  // Source page whose cached videos seed this page's auto-ads candidates. Empty → this page itself.
  autoAdsSourcePageId: z.string().max(120).default(''),
...
export async function fetchPageSettings(
  ...
  return {
    form: {
      ...
      autoAdsSourcePageId: String(data.auto_ads_source_page_id || ''),
...
export async function savePageSettings(
  ...
  const body: Record<string, string> = {
    ...
    auto_ads_source_page_id: form.autoAdsSourcePageId,
